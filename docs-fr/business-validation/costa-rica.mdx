...
export const structuredData = {
	...
	"@type": "TechArticle",
	headline: "Vérification d'entreprise au Costa Rica (NITE) | API KYB Verifik",
	description:
		"Vérifiez les sociétés costariciennes avec le NITE : GET /v2/cr/company avec documentType et documentNumber pour le KYB et la diligence fournisseurs.",
	articleSection: "Documentation API",
	keywords: "vérification d'entreprise, API KYB, validation NITE Costa Rica, vérification de société, source de données officielle, prévention de la fraude",
	about: {
		"@type": "Thing",
...
-   **Nom et numéro NITE** : prend en charge le *Número de Identificación Tributaria de Empresas* (NITE).
-   **Informations sur l'entreprise** : retourne la raison sociale et les détails d'enregistrement.
-   **Correspondance d'entreprise** : confirme que les informations correspondent au numéro NITE.
...
Utilisez cet endpoint pour confirmer les enregistrements **NITE** (*Número de Identificación Tributaria de Empresas*) des entités juridiques costariciennes. Transmettez **`documentType`** `NITE` et le **`documentNumber`** de la société ; la réponse retourne les identifiants commerciaux enregistrés et les champs associés pour le **KYB** et l'onboarding.
...
| ---------------- | ------ | ------- | --------------------------------------------------- |
| `documentType`   | string | **Oui** | Type de document. Valeur autorisée : `NITE`.        |
