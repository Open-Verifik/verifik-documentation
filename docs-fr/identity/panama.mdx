...
export const structuredData = {
	...
	description:
		"Vérifiez les données d'identité nationale CCPA au Panama (document, nom, date de naissance) auprès des sources officielles du registre civil pour le KYC et la prévention de la fraude.",
	articleSection: "Documentation API",
	keywords: "vérification d'identité, API KYC, validation CCPA Panama, source de données officielle, prévention de la fraude",
	about: {
		"@type": "Thing",
...
Vérifiez les données **CCPA** (*Cédula de Identidad Personal*) panaméennes auprès des sources officielles du registre civil. Transmettez **`documentType`** (`CCPA`) et un **`documentNumber`** normalisé ; la réponse confirme que l'enregistrement d'identité est cohérent pour le **KYC**, l'onboarding clients et les contrôles de fraude.
...
5. **Registre civil pré-2006 (AV)** — province immédiatement suivie de `AV`. Exemple : `10AV-1234-12345`.
...
Les règles de format s'alignent sur le validateur open source [cedula-panama](https://github.com/merlos/cedula-panama) (MIT).
