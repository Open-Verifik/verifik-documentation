---
id: mexico-ine
title: Mexique — Validation INE
description: Validez une carte d'électeur mexicaine (INE) auprès du registre électoral à l'aide du CIC, de l'OCR ou de la clave de elector.
...
Validez une **INE** mexicaine (*Credencial para Votar*) auprès du registre électoral. Envoyez `documentType=INE` et un identifiant principal dans `documentNumber`. L'API détecte automatiquement s'il s'agit d'un **CIC** (9 chiffres), d'un **OCR** (13 chiffres) ou d'une **clave de elector** (18 caractères).

Les champs optionnels couvrent les combinaisons par modèle de carte exigées par la source officielle :

- **`citizenIdentifier`** — requis pour les modèles **E, F, G, H** avec le CIC
- **`emissionNumber`** — requis pour le modèle **C** avec OCR et clave de elector
- **`model`** — indice optionnel (`C`–`H`) pour sélectionner la combinaison correcte

Pour une recherche nominative par **CURP**, utilisez **[Citoyen mexicain (CURP)](/verifik-fr/identity/mexico)** (`v2/mx/curp`). Pour extraire des champs depuis des images INE, utilisez **[Mexique — OCR INE](/verifik-fr/identity/mexico-ine-ocr)** (`v2/mx/ine/ocr`).
...
- Related: [Mexican Citizen (CURP)](/verifik-fr/identity/mexico), [Mexico — INE OCR](/verifik-fr/identity/mexico-ine-ocr).
