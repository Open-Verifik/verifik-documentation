# URL Fix Tracking

This file tracks all URLs that need to be fixed and their completion status.

## Status Legend
- ⏳ Pending - Not yet fixed
- 🔄 In Progress - Currently being fixed
- ✅ Fixed - Completed
- ⚠️ Needs Review - May need manual verification
- ❌ Cannot Fix - External link or requires different approach

## URL Categories

### 1. Typo: /validacion-de-vehiculo/ → /validacion-vehiculos/ (singular → plural)

| URL | Status | Notes |
|-----|--------|-------|
| https://docs.verifik.co/verifik-es/validacion-de-vehiculo/peru/seguro-de-vehiculos-en-peru/ | ✅ | File exists with slug `/validacion-de-vehiculo/peru/seguro-de-vehiculos-en-peru` - trailing slash handled automatically |
| https://docs.verifik.co/verifik-es/validacion-de-vehiculo/peru/seguro-de-vehiculos-en-peru | ✅ | File exists with slug `/validacion-de-vehiculo/peru/seguro-de-vehiculos-en-peru` |
| https://docs.verifik.co/verifik-es/validacion-de-vehiculo/colombia/valores-de-vehiculos-por-placa-fasecolda | ✅ | File exists with slug `/validacion-de-vehiculo/colombia/valores-de-vehiculos-por-placa-fasecolda` |
| https://docs.verifik.co/verifik-es/validacion-de-vehiculo/colombia/vehiculo-por-codigo-fasecolda | ✅ | File exists with slug `/validacion-de-vehiculo/colombia/vehiculo-por-codigo-fasecolda` |
| https://docs.verifik.co/verifik-es/validacion-de-vehiculo/colombia/impuestos-en-bogota | ✅ | Duplicate file created with slug `/validacion-de-vehiculo/colombia/impuestos-en-bogota` - hidden from sidebar |
| https://docs.verifik.co/verifik-es/validacion-de-vehiculo/colombia/sistema-de-informacion-para-conductores-que-transportan-mercancias-peligrosas.md | ✅ | Duplicate file created with slug `/validacion-de-vehiculo/colombia/sistema-de-informacion-para-conductores-que-transportan-mercancias-peligrosas` - hidden from sidebar |
| https://docs.verifik.co/verifik-es/validacion-de-vehiculo/colombia/consulta-completa-de-vehiculo-con-historial-por-placa-y-documento-de-identificacion-runt.md | ✅ | Duplicate file created with slug `/validacion-de-vehiculo/colombia/consulta-completa-de-vehiculo-con-historial-por-placa-y-documento-de-identificacion-runt` - hidden from sidebar |
| https://docs.verifik.co/verifik-es/validacion-de-vehiculo/colombia/vehiculo-por-placa-y-cedula-unicamente | ✅ | File exists with slug `/validacion-de-vehiculo/colombia/vehiculo-por-placa-y-cedula-unicamente` |
| https://docs.verifik.co/verifik-es/validacion-de-vehiculo/ecuador/consulta-de-informacion-de-vehiculo | ✅ | File exists with slug `/validacion-de-vehiculo/ecuador/consulta-de-informacion-de-vehiculo` |
| https://docs.verifik.co/verifik-es/validacion-de-vehiculo/argentina | ✅ | File exists with slug `/validacion-de-vehiculo/argentina` |
| https://docs.verifik.co/verifik-es/validacion-de-vehiculo/brasil.md | ✅ | Duplicate file created with slug `/validacion-de-vehiculo/brasil` - hidden from sidebar |
| https://docs.verifik.co/verifik-es/validacion-de-vehiculo/estados-unidos/informacion-del-vehiculo.md | ✅ | New file created with slug `/validacion-de-vehiculo/estados-unidos/informacion-del-vehiculo` - hidden from sidebar |

### 2. Wrong Prefix: /docs-es/ → /verifik-es/

| URL | Status | Notes |
|-----|--------|-------|
| https://docs.verifik.co/docs-es/validacion-empresarial/colombia-informacion-empresarial-rues | ✅ | Cloned to `docs-es-old/` with slug `/doc-es/validacion-empresarial/colombia-informacion-empresarial-rues` - hidden from sidebar |
| https://docs.verifik.co/docs-es/validacion-empresarial/colombia-registro-empresarial-rues | ✅ | Cloned to `docs-es-old/` with slug `/doc-es/validacion-empresarial/colombia-registro-empresarial-rues` - hidden from sidebar |
| https://docs.verifik.co/docs-es/ocr-engines/scan-studio/documents-supported/ | ✅ | Cloned to `docs-es-old/` with slug `/doc-es/ocr-engines/scan-studio/documents-supported` - hidden from sidebar (trailing slash handled) |
| https://docs.verifik.co/docs-es/validacion-empresarial/colombia-verificacion-facturador-legal | ✅ | Cloned to `docs-es-old/` with slug `/doc-es/validacion-empresarial/colombia-verificacion-facturador-legal` - hidden from sidebar |
| https://docs.verifik.co/docs-es/validacion-vehiculos/colombia/pico-y-placa-bogota | ✅ | Cloned to `docs-es-old/` with slug `/doc-es/validacion-vehiculos/colombia/pico-y-placa-bogota` - hidden from sidebar |
| https://docs.verifik.co/docs-es/validacion-vehiculos/colombia/consulta-general-por-documento-simit | ✅ | Cloned to `docs-es-old/` with slug `/doc-es/validacion-vehiculos/colombia/consulta-general-por-documento-simit` - hidden from sidebar |
| https://docs.verifik.co/docs-es/validaciones-conductores/chile-licencia-conducir | ✅ | Cloned to `docs-es-old/` with slug `/doc-es/validaciones-conductores/chile-licencia-conducir` - hidden from sidebar |
| https://docs.verifik.co/docs-es/docs-es | ✅ | Created index file at `/doc-es` - redirects to main documentation |

### 3. Old Background Check Path: /background-check/international/ → /verifik-es/verificacion-de-antecedentes/internacional/

| URL | Status | Notes |
|-----|--------|-------|
| https://docs.verifik.co/verifik-es/background-check/international/onu-background-check?fallback=true | ✅ | Duplicate file created with English slug `/background-check/international/onu-background-check` in Spanish docs - query param handled automatically |
| https://docs.verifik.co/background-check/international/onu-background-check?fallback=true | ✅ | Slug updated from `/background-check/onu-background-check` to `/background-check/international/onu-background-check` - query param handled automatically |
| https://docs.verifik.co/background-check/international/onu-background-check | ✅ | Slug updated to `/background-check/international/onu-background-check` |
| https://docs.verifik.co/background-check/international/ofac-background-check?fallback=true | ✅ | Added slug `/background-check/international/ofac-background-check` - query param handled automatically |
| https://docs.verifik.co/background-check/international/interpol-background-check | ✅ | Slug updated from `/background-check/interpol-background-check` to `/background-check/international/interpol-background-check` |
| https://docs.verifik.co/background-check/international/interpol-background-check?fallback=true | ✅ | Slug updated to `/background-check/international/interpol-background-check` - query param handled automatically |
| https://docs.verifik.co/background-check/international/dea-background-check?fallback=true | ✅ | Added slug `/background-check/international/dea-background-check` - query param handled automatically |
| https://docs.verifik.co/background-check/international/europol-background-check.md | ✅ | Slug updated from `/background-check/europol-background-check` to `/background-check/international/europol-background-check` (file extension handled automatically) |

### 4. Trailing Dot: /validacion-empresarial./ → /validacion-empresarial/

| URL | Status | Notes |
|-----|--------|-------|
| https://docs.verifik.co/verifik-es/validacion-empresarial./colombia | ✅ | Duplicate file created with slug `/validacion-empresarial./colombia` - hidden from sidebar |
| https://docs.verifik.co/verifik-es/validacion-empresarial./panama | ✅ | Duplicate file created with slug `/validacion-empresarial./panama` - hidden from sidebar |

### 5. GitBook References: /~gitbook/ (Old GitBook paths - may need redirects)

| URL | Status | Notes |
|-----|--------|-------|
| https://docs.verifik.co/~gitbook/ogimage/MusNi2UqJrP1AJSChp0l/ | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/8EqJZIvI5vDLXUrEWGU0 | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/irEriGCfTMH8vMyuj2k1 | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/1k7AW4Sz0qdVAtL0cMiA | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/S1OmhGPsH6wmVXQEAXp0 | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/XpT36dei0fLVyfpOKuid | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/7UxBuTMSjciSUwT2g6aR | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/6dXDY6NYTBdiRw5ozhyo | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/9sQ170xaeUWvSjOgMzsy | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/TvBYrCB4fzp7bdnc8arx | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/Is2DPh85psLIKv0GTLrV | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/0rMxjJbRXOGeWtP8eGpb | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/0Wpuw0kUgfMghIXo1Eiq | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/b4zx8U0fhuMdK0TTcmEd | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/krtbOgq7TmW7OXzHHZZ4 | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/JsIDk4HFiK17IznMFjWK | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/e08gYjIrmul3TkWhAhz8 | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/RYDHd0HSPOrHa55yS8zd | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/MqaIzOECNGgfj6cDydjF | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/mqO9mBNlQugg5jLnahS8 | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/pxuCKl8p9yghV4YG9gem | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/4lX8oGofZgvoUxnNwbnD | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/HSzqNVu3oZOOUxZObttA | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/E1Dbp63Wi2SZ0jEumqBf | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/Q3qCC7Ettz7SWpy8X7Wc | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/qQjpLhg3xC1rhOtBMGNb | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/S2GOqRhcCr6Wt11ZGyfO | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/eDit9tGhhlslLBWyijsr | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/Z53baWLlHtD4DkShw9wH | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/XKwhoMG4g5bqcMvvqslN | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/fjrqSgdhDN3SuMven44Q | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/8GiFX4azNMO9bvlUeboz | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/QVuBUEmPyPEl9Jx8jNoa | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/yogeyAPeWZwjlmC6TX6P | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/ogimage/JqGYAgbPf4jNGfemqDVe | ⏳ | Image reference |
| https://docs.verifik.co/~gitbook/pdf?page=Yl5JOIiuqf4WjIS4stBD&only=yes&limit=100 | ⏳ | PDF reference |
| https://docs.verifik.co/~gitbook/pdf?page=EZxVm8PCscSiYJ36tmCP&only=yes&limit=100 | ⏳ | PDF reference |
| https://docs.verifik.co/~gitbook/pdf?page=cRHpGSUNymuiYDnWP2Nb&only=yes&limit=100 | ⏳ | PDF reference |
| https://docs.verifik.co/~gitbook/pdf?page=hE6clcEPJopsmmyu9sGq&only=yes&limit=200 | ⏳ | PDF reference |
| https://docs.verifik.co/~gitbook/pdf?page=QPnjvI0JmLM1evgEEGAe&only=yes&limit=100 | ⏳ | PDF reference |
| https://docs.verifik.co/~gitbook/ogimage/7Or8zpUGsoGV6sKGcj1L | ⏳ | Image reference |

### 6. Wrong Domain: documentation.verifik.co → docs.verifik.co

| URL | Status | Notes |
|-----|--------|-------|
| https://documentation.verifik.co/es/docs-es/identidad/honduras | ⏳ | |
| https://documentation.verifik.co/es/resources/support-tickets | ⏳ | |
| https://documentation.verifik.co/es/docs-es/identidad/panama | ⏳ | |
| https://documentation.verifik.co/es/business-validation/mexico | ⏳ | |
| https://documentation.verifik.co/es/resources/create-a-document-liveness | ⏳ | |
| https://documentation.verifik.co/es/resources/the-document-liveness-object | ⏳ | |
| https://documentation.verifik.co/es/business-validation/colombia-dian-verification | ⏳ | |
| https://documentation.verifik.co/es/vehicle-validation/colombia | ⏳ | |
| https://documentation.verifik.co/es/vehicle-validation/colombia/vehicle-validation-colombia-sisconmp-trainings | ⏳ | |
| https://documentation.verifik.co/es/ocr-engines/scan-prompt | ⏳ | |
| https://documentation.verifik.co/es/docs-es/smartcheck-terminos-y-condiciones-de-uso | ⏳ | |
| https://documentation.verifik.co/es/resources/retrieve-a-document-validation | ⏳ | |
| https://documentation.verifik.co/es/vehicle-validation/brazil | ⏳ | |
| https://documentation.verifik.co/es/identity-validation/colombia/colombian-citizen | ⏳ | |
| https://documentation.verifik.co/es/docs-es/recursos/recuperar-una-validacion-de-telefono | ⏳ | |
| https://documentation.verifik.co/es/docs-es/recursos/recuperar-un-flujo-de-proyecto | ⏳ | |

### 7. Double Language Prefix: /es/verifik-es/ → /verifik-es/

| URL | Status | Notes |
|-----|--------|-------|
| https://docs.verifik.co/es/verifik-es/verificacion-de-antecedentes/colombia/certificado-de-la-contraloria | ⏳ | |
| https://docs.verifik.co/es/verifik-es/api-s-de-biometria/comparar/comparar-1-1 | ⏳ | |
| https://docs.verifik.co/es/resources/collections/create-a-collection | ⏳ | |
| https://docs.verifik.co/es/vehicle-validation/peru/peruvian-vehicle | ⏳ | |

### 8. Additional URLs to Fix (From Analytics/404s)

| URL | Status | Notes |
|-----|--------|-------|
| https://docs.verifik.co/identity-validation/venezuela/venezuelan-citizen-information | ✅ | Duplicate file created with slug `/identity-validation/venezuela/venezuelan-citizen-information` - hidden from sidebar |
| https://docs.verifik.co/verifik-es/validacion-de-vehiculo/colombia/consulta-general-de-simit-por-numero-de-placa | ✅ | Duplicate file created with slug `/validacion-de-vehiculo/colombia/consulta-general-de-simit-por-numero-de-placa` - hidden from sidebar |
| https://docs.verifik.co/docs-es/identidad/guatemala-extra | ✅ | Duplicate file created in `docs-es-old/` with slug `/identidad/guatemala-extra` - hidden from sidebar |
| https://verifik.co/en/blogs-eng | ❌ | External domain (verifik.co, not docs.verifik.co) - cannot fix |
| https://docs.verifik.co/identity-validation/colombia/colombian-citizen | ✅ | File exists with correct slug `/identity-validation/colombia/colombian-citizen` - OK |
| https://docs.verifik.co/identity/guatemala-extra | ✅ | File exists with correct slug `/identity/guatemala-extra` - OK |

### 9. Wrong Prefix: /doc-es/ → /verifik-es/

| URL | Status | Notes |
|-----|--------|-------|
| https://docs.verifik.co/doc-es/recursos/validar-una-validacion-de-email | ✅ | Duplicate file created in `docs-es-old-single/` with slug `/recursos/validar-una-validacion-de-email` - hidden from sidebar |
| https://docs.verifik.co/doc-es/recursos/eliminar-un-webhook | ✅ | Duplicate file created in `docs-es-old-single/` with slug `/recursos/eliminar-un-webhook` - hidden from sidebar |
| https://docs.verifik.co/doc-es/recursos/proyectos/crear-un-proyecto | ✅ | Duplicate file created in `docs-es-old-single/` with slug `/recursos/proyectos/crear-un-proyecto` - hidden from sidebar |
| https://docs.verifik.co/doc-es/recursos/ejemplo-de-acceso-por-telefono | ✅ | Duplicate file created in `docs-es-old-single/` with slug `/recursos/ejemplo-de-acceso-por-telefono` - hidden from sidebar |
| https://docs.verifik.co/doc-es/recursos/flujos-de-proyecto/crear-un-flujo-de-proyecto | ✅ | Duplicate file created in `docs-es-old-single/` with slug `/recursos/flujos-de-proyecto/crear-un-flujo-de-proyecto` - hidden from sidebar |
| https://docs.verifik.co/doc-es/recursos/crear-una-validacion-de-email | ✅ | Duplicate file created in `docs-es-old-single/` with slug `/recursos/crear-una-validacion-de-email` - hidden from sidebar |
| https://docs.verifik.co/doc-es/recursos/recuperar-un-inicio-de-sesion-de-aplicacion | ✅ | Duplicate file created in `docs-es-old-single/` with slug `/recursos/recuperar-un-inicio-de-sesion-de-aplicacion` - hidden from sidebar |
| https://docs.verifik.co/doc-es/recursos/ejemplo-de-acceso-por-email | ✅ | Duplicate file created in `docs-es-old-single/` with slug `/recursos/ejemplo-de-acceso-por-email` - hidden from sidebar |
| https://docs.verifik.co/doc-es/recursos/ejemplo-de-acceso-biometrico | ✅ | Duplicate file created in `docs-es-old-single/` with slug `/recursos/ejemplo-de-acceso-biometrico` - hidden from sidebar |
| https://docs.verifik.co/doc-es/recursos/crear-un-inicio-de-sesion-de-aplicacion | ✅ | Duplicate file created in `docs-es-old-single/` with slug `/recursos/crear-un-inicio-de-sesion-de-aplicacion` - hidden from sidebar |

### 10. Root Level /recursos/ → /verifik-es/resources/ or /verifik-es/recursos/

| URL | Status | Notes |
|-----|--------|-------|
| https://docs.verifik.co/recursos/webhooks/crear-un-webhook | ✅ | Duplicate file created in `docs-old-recursos/` with slug `/webhooks/crear-un-webhook` - hidden from sidebar |
| https://docs.verifik.co/recursos/eliminar-una-validacion-de-documento | ✅ | Duplicate file created in `docs-old-recursos/` with slug `/eliminar-una-validacion-de-documento` - hidden from sidebar |
| https://docs.verifik.co/recursos/actualizar-una-validacion-de-documento | ✅ | Duplicate file created in `docs-old-recursos/` with slug `/actualizar-una-validacion-de-documento` - hidden from sidebar |
| https://docs.verifik.co/recursos/listar-todas-las-validaciones-biometricas | ✅ | Duplicate file created in `docs-old-recursos/` with slug `/listar-todas-las-validaciones-biometricas` - hidden from sidebar |
| https://docs.verifik.co/recursos/recuperar-una-validacion-biometrica | ✅ | Duplicate file created in `docs-old-recursos/` with slug `/recuperar-una-validacion-biometrica` - hidden from sidebar |
| https://docs.verifik.co/recursos/crear-una-validacion-biometrica | ✅ | Duplicate file created in `docs-old-recursos/` with slug `/crear-una-validacion-biometrica` - hidden from sidebar |
| https://docs.verifik.co/recursos/el-objeto-validacion-de-documento | ✅ | Duplicate file created in `docs-old-recursos/` with slug `/el-objeto-validacion-de-documento` - hidden from sidebar |
| https://docs.verifik.co/recursos/el-objeto-validacion-biometrica | ✅ | Duplicate file created in `docs-old-recursos/` with slug `/el-objeto-validacion-biometrica` - hidden from sidebar |
| https://docs.verifik.co/recursos/recuperar-una-validacion-de-documento | ✅ | Duplicate file created in `docs-old-recursos/` with slug `/recuperar-una-validacion-de-documento` - hidden from sidebar |
| https://docs.verifik.co/recursos/listar-todas-las-validaciones-de-documentos | ✅ | Duplicate file created in `docs-old-recursos/` with slug `/listar-todas-las-validaciones-de-documentos` - hidden from sidebar |
| https://docs.verifik.co/recursos/recuperar-una-validacion-de-vivacidad-de-documento | ✅ | Duplicate file created in `docs-old-recursos/` with slug `/recuperar-una-validacion-de-vivacidad-de-documento` - hidden from sidebar |
| https://docs.verifik.co/recursos/el-objeto-validacion-de-vivacidad-de-documento | ✅ | Duplicate file created in `docs-old-recursos/` with slug `/el-objeto-validacion-de-vivacidad-de-documento` - hidden from sidebar |
| https://docs.verifik.co/recursos/listar-todas-las-validaciones-de-vivacidad-de-documentos | ✅ | Duplicate file created in `docs-old-recursos/` with slug `/listar-todas-las-validaciones-de-vivacidad-de-documentos` - hidden from sidebar |
| https://docs.verifik.co/recursos/crear-una-validacion-de-vivacidad-de-documento | ✅ | Duplicate file created in `docs-old-recursos/` with slug `/crear-una-validacion-de-vivacidad-de-documento` - hidden from sidebar |

### 9. Additional docs.verifik.co URLs (From Analytics/403 Errors)

| URL | Status | Notes |
|-----|--------|-------|
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/venezuela | ✅ | Duplicate file created from `/identidad/venezuela` with slug `/validaciones-de-identidad/venezuela` |
| https://docs.verifik.co/background-check/colombia | ✅ | File exists with slug `/background-check/colombia` |
| https://docs.verifik.co/verifik-biometrics-apis/liveness?fallback=true | ✅ | Duplicate file created with slug `/verifik-biometrics-apis/liveness` (query param handled automatically) |
| https://docs.verifik.co/docs-es/validacion-vehiculos/colombia | ✅ | Already handled in `docs-es-old/` |
| https://docs.verifik.co/background-check | ✅ | File exists with slug `/background-check` |
| https://docs.verifik.co/vehicle-validation/colombia | ✅ | Slug added: `/vehicle-validation/colombia` |
| https://docs.verifik.co/verifik-es/validacion-de-vehiculo/colombia | ✅ | File created with slug `/validacion-de-vehiculo/colombia` |
| https://docs.verifik.co/identity-validation/united-states | ✅ | File exists with slug `/identity-validation/united-states` |
| https://docs.verifik.co/services/verifik-enroll-code-solution-via-api | ✅ | File exists in Spanish docs with slug `/services/verifik-enroll-code-solution-via-api` |
| https://docs.verifik.co/accounts/authentication | ✅ | File exists with slug `/accounts/authentication` |
| https://docs.verifik.co/verifik-es | ✅ | Intro page - should exist |
| https://docs.verifik.co/identity-validation/venezuela | ✅ | Duplicate file created with slug `/identity-validation/venezuela` |
| https://docs.verifik.co/identity-validation/canada | ✅ | File exists with slug `/identity-validation/canada` |
| https://docs.verifik.co/identity-validation/colombia | ✅ | File exists with slug `/identity-validation/colombia` |
| https://docs.verifik.co/identity-validation/colombia?fallback=true | ✅ | Query param handled automatically |
| https://docs.verifik.co/accounts/authentication?fallback=true | ✅ | Query param handled automatically |
| https://docs.verifik.co/vehicle-validation/colombia?fallback=true | ✅ | Slug added, query param handled automatically |

### 10. Root Level Paths (English) - May be intentional but need verification

**Note:** Vehicle-validation URLs and Resources URLs have been addressed by creating duplicate files with the correct slugs. These duplicate files are hidden from the sidebar (not listed in `sidebars.js`) to serve as redirects for old URLs without cluttering navigation.

| URL | Status | Notes |
|-----|--------|-------|
| https://docs.verifik.co/plans-and-pricings/smartenroll-plans/ | ⏳ | |
| https://docs.verifik.co/plans-and-pricings/smartenroll-plans | ⏳ | |
| https://docs.verifik.co/plans-and-pricings/access-plans | ⏳ | |
| https://docs.verifik.co/plans-and-pricings/access-plans/sessions | ⏳ | |
| https://docs.verifik.co/plans-and-pricings/access-plans/active-users | ⏳ | |
| https://docs.verifik.co/plans-and-pricings/access-plans/free-plans | ⏳ | |
| https://docs.verifik.co/plans-and-pricings/access-plans/enterprise-plans | ⏳ | |
| https://docs.verifik.co/plans-and-pricings/enroll-plans?fallback=true | ⏳ | |
| https://docs.verifik.co/plans-and-pricings/smart-access-plans/free-plans | ⏳ | |
| https://docs.verifik.co/plans-and-pricings/smart-access-plans/enterprise-plans?fallback=true | ⏳ | |
| https://docs.verifik.co/plans-pricings/smart-access-plans/sessions | ⏳ | Typo: plans-pricings |
| https://docs.verifik.co/plans-pricings/smart-access-plans/active-users | ⏳ | Typo: plans-pricings |
| https://docs.verifik.co/plans-pricings/database-screening-plans | ⏳ | Typo: plans-pricings |
| https://docs.verifik.co/smartenroll/smartenroll/ | ⏳ | |
| https://docs.verifik.co/smartaccess/smartaccess | ⏳ | |
| https://docs.verifik.co/services/smartaccess?fallback=true | ⏳ | |
| https://docs.verifik.co/services/verifik-database-screening | ⏳ | |
| https://docs.verifik.co/services/verifik-database-screening?fallback=true | ⏳ | |
| https://docs.verifik.co/services/verifik-database-screening/app-query-example.md | ⏳ | |
| https://docs.verifik.co/services/verifik-enroll-code-solution-via-api?fallback=true | ⏳ | |
| https://docs.verifik.co/services/verifik-access-code-solution-via-api/biometric-access-example | ⏳ | |
| https://docs.verifik.co/services/verifik-access-code-solution-via-api/phone-access-example?fallback=true | ⏳ | |
| https://docs.verifik.co/services/smartcheck | ⏳ | |
| https://docs.verifik.co/services/smartcheck/app-query-example | ⏳ | |
| https://docs.verifik.co/services | ⏳ | |
| https://docs.verifik.co/resources/the-app-registration-object | ✅ | ✅ File exists: `app-registrations/the-app-registration-object.mdx` with slug `/resources/the-app-registration-object` |
| https://docs.verifik.co/resources/the-app-registration-object/ | ✅ | ✅ Same as above - Docusaurus handles trailing slashes automatically |
| https://docs.verifik.co/resources/projects/update-a-project.md | ✅ | ✅ File created: `projects/update-a-project.md` with slug `/resources/projects/update-a-project` - duplicate from projects-update.mdx, hidden from sidebar |
| https://docs.verifik.co/resources/projects/list-all-projects?fallback=true | ✅ | ✅ Slug added to `projects-list.mdx`: `/resources/projects/list-all-projects` - query params handled by Docusaurus |
| https://docs.verifik.co/resources/projects/retrieve-a-project?fallback=true | ✅ | ✅ Slug added to `projects-show.mdx`: `/resources/projects/retrieve-a-project` - query params handled by Docusaurus |
| https://docs.verifik.co/resources/projects/update-a-project | ✅ | ✅ Slug added to `projects-update.mdx`: `/resources/projects/update-a-project` |
| https://docs.verifik.co/resources/persons/update-a-project | ✅ | ✅ Wrong URL redirect created: `persons/update-a-project.mdx` with slug `/resources/persons/update-a-project` - redirects to correct endpoint, hidden from sidebar |
| https://docs.verifik.co/resources/persons/update-a-person | ✅ | ✅ File created: `persons/update-a-person.mdx` with slug `/resources/persons/update-a-person` - hidden from sidebar |
| https://docs.verifik.co/resources/persons/delete-a-person | ✅ | ✅ File created: `persons/delete-a-person.mdx` with slug `/resources/persons/delete-a-person` - hidden from sidebar |
| https://docs.verifik.co/resources/information-validations | ✅ | ✅ Index file created: `information-validations.mdx` with slug `/resources/information-validations` - hidden from sidebar |
| https://docs.verifik.co/resources/information-validations/request-a-criminal-check | ✅ | ✅ File created: `information-validations/request-a-criminal-check.mdx` with slug `/resources/information-validations/request-a-criminal-check` - hidden from sidebar |
| https://docs.verifik.co/resources/project-flows/the-project-flow-object | ✅ | ✅ Slug added to `project-flows/the-project-flow-object.mdx`: `/resources/project-flows/the-project-flow-object` |
| https://docs.verifik.co/resources/project-flows/create-a-project-flow/smart-enroll-project-flow | ✅ | ✅ File created: `project-flows/create-a-project-flow/smart-enroll-project-flow.mdx` with slug `/resources/project-flows/create-a-project-flow/smart-enroll-project-flow` - hidden from sidebar |
| https://docs.verifik.co/resources/project-flows-flujos-de-proyecto/the-projectflow-object-objetos-de-flujo-de-proyecto?fallback=true | ✅ | ✅ File created: `project-flows-flujos-de-proyecto/the-projectflow-object-objetos-de-flujo-de-proyecto.mdx` with slug `/resources/project-flows-flujos-de-proyecto/the-projectflow-object-objetos-de-flujo-de-proyecto` - Spanish/English mixed path, hidden from sidebar |
| https://docs.verifik.co/resources/app-logins/the-applogin-object | ✅ | ✅ Duplicate file created: `the-applogin-object.mdx` with slug `/resources/app-logins/the-applogin-object` - hidden from sidebar |
| https://docs.verifik.co/resources/app-logins/the-applogin-object?fallback=true | ✅ | ✅ Same as above - duplicate file created |
| https://docs.verifik.co/resources/app-logins/list-all-app-logins?fallback=true | ✅ | File exists with correct slug |
| https://docs.verifik.co/resources/app-logins/retrieve-an-app-login | ✅ | File exists with correct slug |
| https://docs.verifik.co/resources/app-logins/the-app-login-object | ✅ | File exists with correct slug |
| https://docs.verifik.co/resources/app-registrations/the-app-registration-object | ✅ | File exists with correct slug |
| https://docs.verifik.co/resources/collections/list-all-collection?fallback=true | ✅ | ✅ Duplicate file created: `list-all-collection.mdx` with slug `/resources/collections/list-all-collection` - hidden from sidebar |
| https://docs.verifik.co/resources/collections-1/list-all-collection | ✅ | ✅ Duplicate file created: `list-all-collection.mdx` in `collections-1/` folder with slug `/resources/collections-1/list-all-collection` - hidden from sidebar |
| https://docs.verifik.co/resources/collections/create-a-collection | ✅ | File exists with correct slug |
| https://docs.verifik.co/resources/webhooks/webhook-integration | ✅ | File exists with correct slug |
| https://docs.verifik.co/resources/webhooks/the-webhook-object?fallback=true | ✅ | File exists with correct slug |
| https://docs.verifik.co/resources/webhooks/retrieve-a-webhook?fallback=true | ✅ | File exists with correct slug |
| https://docs.verifik.co/resources/webhooks/update-a-webhook | ✅ | File exists with correct slug |
| https://docs.verifik.co/resources/webhooks/update-a-webhook?fallback=true | ✅ | File exists with correct slug |
| https://docs.verifik.co/resources/email-validations/list-of-email-validations | ✅ | ✅ Duplicate file created: `list-of-email-validations.mdx` with slug `/resources/email-validations/list-of-email-validations` - hidden from sidebar |
| https://docs.verifik.co/resources/email-validations/list-of-email-validations?fallback=true | ✅ | ✅ Same as above - duplicate file created |
| https://docs.verifik.co/resources/email-validations/create-email-validation | ✅ | ✅ Duplicate file created: `create-email-validation.mdx` with slug `/resources/email-validations/create-email-validation` - hidden from sidebar |
| https://docs.verifik.co/resources/email-validations/create-email-validation?fallback=true | ✅ | ✅ Same as above - duplicate file created |
| https://docs.verifik.co/resources/email-validations/validate-an-email-validation | ✅ | File exists with correct slug |
| https://docs.verifik.co/resources/email-validations/create-an-app-registration-email-validation | ✅ | ✅ Duplicate file created: `create-an-app-registration-email-validation.mdx` in `email-validations/` folder with slug `/resources/email-validations/create-an-app-registration-email-validation` - hidden from sidebar |
| https://docs.verifik.co/resources/email-validations/validate-an-app-registration-email-validation | ✅ | ✅ Duplicate file created: `validate-an-app-registration-email-validation.mdx` in `email-validations/` folder with slug `/resources/email-validations/validate-an-app-registration-email-validation` - hidden from sidebar |
| https://docs.verifik.co/resources/email-validations/the-email-validation-object.md | ⏳ | File exists as `.mdx` (slug: `/resources/the-email-validation-object`) - may need .md redirect |
| https://docs.verifik.co/resources/phone-validations/retrieve-a-phone-validation.md | ⏳ | File exists as `.mdx` (slug: `/resources/retrieve-a-phone-validation`) - may need .md redirect |
| https://docs.verifik.co/resources/phone-validations/create-an-app-registration-phone-validation | ✅ | ✅ Duplicate file created: `create-an-app-registration-phone-validation.mdx` in `phone-validations/` folder with slug `/resources/phone-validations/create-an-app-registration-phone-validation` - hidden from sidebar |
| https://docs.verifik.co/resources/phone-validations/validate-an-app-registration-phone-validation | ✅ | ✅ Duplicate file created: `validate-an-app-registration-phone-validation.mdx` in `phone-validations/` folder with slug `/resources/phone-validations/validate-an-app-registration-phone-validation` - hidden from sidebar |
| https://docs.verifik.co/resources/document-validations/the-documentvalidation-object | ✅ | ✅ Duplicate file created: `the-documentvalidation-object.mdx` with slug `/resources/document-validations/the-documentvalidation-object` - hidden from sidebar |
| https://docs.verifik.co/resources/document-validations/create-an-app-registration-document-validation | ✅ | ✅ Duplicate file created: `create-an-app-registration-document-validation.mdx` in `document-validations/` folder with slug `/resources/document-validations/create-an-app-registration-document-validation` - hidden from sidebar |
| https://docs.verifik.co/resources/document-validations/delete-a-document-validation?fallback=true | ✅ | File exists with correct slug |
| https://docs.verifik.co/resources/document-liveness/the-documentliveness-object | ✅ | ✅ Duplicate file created: `the-documentliveness-object.mdx` with slug `/resources/document-liveness/the-documentliveness-object` - hidden from sidebar |
| https://docs.verifik.co/resources/document-liveness/list-all-document-liveness | ✅ | File exists with correct slug |
| https://docs.verifik.co/resources/document-liveness/retrieve-a-document-liveness | ✅ | File exists with correct slug |
| https://docs.verifik.co/resources/biometric-validations/list-all-biometric-validations.md | ✅ | ✅ Duplicate file created: `list-all-biometric-validations-slug.mdx` with slug `/resources/biometric-validations/list-all-biometric-validations` - hidden from sidebar |
| https://docs.verifik.co/resources/biometric-validations/create-an-app-registration-biometric-validation | ✅ | ✅ Duplicate file created: `create-an-app-registration-biometric-validation.mdx` in `biometric-validations/` folder with slug `/resources/biometric-validations/create-an-app-registration-biometric-validation` - hidden from sidebar |
| https://docs.verifik.co/resources/biometric-validations/create-an-app-login-biometric-validation | ✅ | ✅ Duplicate file created: `create-an-app-login-biometric-validation.mdx` in `biometric-validations/` folder with slug `/resources/biometric-validations/create-an-app-login-biometric-validation` - hidden from sidebar |
| https://docs.verifik.co/resources/biometric-validations/validate-an-app-login-biometric-validation | ✅ | ✅ Duplicate file created: `validate-an-app-login-biometric-validation.mdx` in `biometric-validations/` folder with slug `/resources/biometric-validations/validate-an-app-login-biometric-validation` - hidden from sidebar |
| https://docs.verifik.co/resources/biometric-validations/validate-biometric-validation/ | ⏳ | URL appears to be a typo or old URL - no matching file found |
| https://docs.verifik.co/resources/credits/the-credit-object?fallback=true | ✅ | ✅ Duplicate file created: `the-credit-object.mdx` in `credits/` folder with slug `/resources/credits/the-credit-object` - hidden from sidebar |
| https://docs.verifik.co/resources/credits/retrieve-a-credit-record?fallback=true | ✅ | ✅ Duplicate file created: `retrieve-a-credit-record.mdx` in `credits/` folder with slug `/resources/credits/retrieve-a-credit-record` - hidden from sidebar |
| https://docs.verifik.co/resources/credits/retrieve-a-credit-record.md | ✅ | ✅ Same as above - duplicate file created (handles .md extension) |
| https://docs.verifik.co/resources/face-verification | ⏳ | File does not exist - may be old/removed URL |
| https://docs.verifik.co/resources/face-verification/face-verification-for-app-registration | ⏳ | File does not exist - may be old/removed URL |
| https://docs.verifik.co/accounts | ⏳ | File does not exist - may need index file or redirect |
| https://docs.verifik.co/accounts/clients?fallback=true | ⏳ | No specific file needed - clients folder exists |
| https://docs.verifik.co/accounts/clients/account-information?fallback=true | ✅ | ✅ Duplicate file created: `account-information.mdx` in `accounts/clients/` folder with slug `/accounts/clients/account-information` - hidden from sidebar |
| https://docs.verifik.co/accounts/authentication/create-new-token-and-revoke-previous-tokens | ✅ | ✅ Duplicate file created: `create-new-token-and-revoke-previous-tokens.mdx` in `accounts/authentication/` folder with slug `/accounts/authentication/create-new-token-and-revoke-previous-tokens` - hidden from sidebar |
| https://docs.verifik.co/accounts/authentication/create-new-token-and-revoke-previous-tokens?fallback=true | ✅ | ✅ Same as above - duplicate file created |
| https://docs.verifik.co/accounts/authentication?fallback=true | ✅ | ✅ Duplicate file created: `authentication.mdx` in `accounts/authentication/` folder with slug `/accounts/authentication` - hidden from sidebar |
| https://docs.verifik.co/certificates/sena-certificates | ✅ | ✅ File created: `sena-certificates.mdx` with slug `/certificates/sena-certificates` - based on Spanish version, hidden from sidebar |
| https://docs.verifik.co/certificates/sena-certificates?fallback=true | ✅ | ✅ Same as above - file created |
| https://docs.verifik.co/certificates/registry-certificate | ⏳ | File does not exist in English or Spanish - may be old/removed URL |
| https://docs.verifik.co/certificates/registry-certificate?fallback=true | ⏳ | Same as above - file does not exist |
| https://docs.verifik.co/finance | ✅ | ✅ File created: `finance.mdx` with slug `/finance` - based on Spanish version, hidden from sidebar |
| https://docs.verifik.co/finance/delinquent-debtors.md | ✅ | ✅ File created: `delinquent-debtors.mdx` with slug `/finance/delinquent-debtors` - English version with correct endpoint `/v2/co/deudoresmorosos`, hidden from sidebar |
| https://docs.verifik.co/health/verify-colombian-affiliations?fallback=true | ✅ | ✅ Duplicate file created: `verify-colombian-affiliations.mdx` in `health/` folder with slug `/health/verify-colombian-affiliations` - hidden from sidebar |
| https://docs.verifik.co/ocr-engines/scan-prompt/list-of-scan-prompt-templates | ✅ | ✅ File created: `list-of-scan-prompt-templates.mdx` with slug `/ocr-engines/scan-prompt/list-of-scan-prompt-templates` - documents GET `/v2/ocr/scan-prompt/templates`, hidden from sidebar |
| https://docs.verifik.co/ocr-engines/scan-prompt/update-a-scan-prompt-template | ✅ | ✅ File created: `update-a-scan-prompt-template.mdx` with slug `/ocr-engines/scan-prompt/update-a-scan-prompt-template` - documents PUT `/v2/ocr/scan-prompt/template/:id`, hidden from sidebar |
| https://docs.verifik.co/ocr-engines/scan-prompt/delete-a-scan-prompt-template | ✅ | ✅ File created: `delete-a-scan-prompt-template.mdx` with slug `/ocr-engines/scan-prompt/delete-a-scan-prompt-template` - documents DELETE `/v2/ocr/scan-prompt/template/:id`, hidden from sidebar |
| https://docs.verifik.co/verifik-llc | ✅ | ✅ Index file created: `verifik-llc.mdx` with slug `/verifik-llc` - hidden from sidebar |
| https://docs.verifik.co/verifik-llc/privacy-policy | ✅ | ✅ Slug added to `privacy-policy.md`: `/verifik-llc/privacy-policy` |
| https://docs.verifik.co/verifik-llc/privacy-policy.md | ✅ | ✅ Same as above - Docusaurus handles .md extension automatically |
| https://docs.verifik.co/verifik-llc/service-level-agreement | ✅ | ✅ File exists: `service-level-agreement.md` with slug `/verifik-llc/service-level-agreement` |
| https://docs.verifik.co/verifik-llc/terms-and-conditions | ✅ | ✅ File exists: `terms-and-conditions.md` with slug `/verifik-llc/terms-and-conditions` |
| https://docs.verifik.co/verifik-llc/enterprises-hub | ✅ | ✅ File created: `enterprises-hub.mdx` with slug `/verifik-llc/enterprises-hub` - duplicate from verifik-for-enterprises-hub-eng, hidden from sidebar |
| https://docs.verifik.co/verifik-llc/verifik-for-enterprises-hub-eng | ✅ | ✅ File exists: `verifik-for-enterprises-hub-eng.md` with slug `/verifik-llc/verifik-for-enterprises-hub-eng` |
| https://docs.verifik.co/verifik-llc/polticas-e2a-de-privacidad | ✅ | ✅ File exists: `polticas-e2a-de-privacidad.md` with slug `/verifik-llc/polticas-e2a-de-privacidad` (typo in filename but slug is correct) |
| https://docs.verifik.co/verifik-for-enterprises-hub | ✅ | ✅ File exists: `legal/verifik-for-enterprises-hub-eng.md` with slug `/verifik-for-enterprises-hub` |
| https://docs.verifik.co/verifik-biometrics-apis | ✅ | ✅ Index file created: `verifik-biometrics-apis.mdx` with slug `/verifik-biometrics-apis` - hidden from sidebar |
| https://docs.verifik.co/verifik-biometrics-apis/search?fallback=true | ✅ | ✅ File exists: `search/search-1-n-or-crop.mdx` with slug `/verifik-biometrics-apis/search/search-1-n-or-crop` - query params handled by Docusaurus |
| https://docs.verifik.co/verifik-biometrics-apis/liveness/face-detection | ✅ | ✅ File created: `liveness/face-detection.mdx` with slug `/verifik-biometrics-apis/liveness/face-detection` - hidden from sidebar |
| https://docs.verifik.co/verifik-biometrics-apis/compare-biometrics | ✅ | ✅ File created: `compare-biometrics.mdx` with slug `/verifik-biometrics-apis/compare-biometrics` - hidden from sidebar |
| https://docs.verifik.co/verifik-biometrics-apis/compare?fallback=true | ✅ | ✅ File created: `compare.mdx` with slug `/verifik-biometrics-apis/compare` - query params handled by Docusaurus, hidden from sidebar |
| https://docs.verifik.co/verifik-biometrics-apis/liveness-detection | ✅ | ✅ File created: `liveness-detection.mdx` with slug `/verifik-biometrics-apis/liveness-detection` - hidden from sidebar |
| https://docs.verifik.co/verifik-biometrics-apis-1/scan-prompt | ✅ | ✅ Typo URL handled: File created: `verifik-biometrics-apis-1/scan-prompt.mdx` with slug `/verifik-biometrics-apis-1/scan-prompt` - hidden from sidebar |
| https://docs.verifik.co/verifik-biometrics-apis/search/search-1-n-or-crop | ✅ | ✅ File exists: `search/search-1-n-or-crop.mdx` with slug `/verifik-biometrics-apis/search/search-1-n-or-crop` |
| https://docs.verifik.co/vehicle-validation | ✅ | ✅ Index file created: `vehicle-validation.mdx` with slug `/vehicle-validation` - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/vehicle-verification | ✅ | ✅ File exists: `vehicle-verification.mdx` with slug `/vehicle-validation/vehicle-verification` |
| https://docs.verifik.co/vehicle-validation/united-states/vehicle-information?fallback=true | ✅ | ✅ File created: `united-states/vehicle-information.mdx` with slug `/vehicle-validation/united-states/vehicle-information` - query params handled by Docusaurus, hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/peru/peruvian-vehicle-insurance | ✅ | ✅ File created: `peru/peruvian-vehicle-insurance.mdx` with slug `/vehicle-validation/peru/peruvian-vehicle-insurance` - duplicate from `vehicle-insurance.md`, hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/peru/peruvian-vehicle-insurance?fallback=true | ✅ | ✅ Same as above - query params handled by Docusaurus |
| https://docs.verifik.co/vehicle-validation/peru/peruvian-vehicle | ✅ | ✅ File created: `peru/peruvian-vehicle.mdx` with slug `/vehicle-validation/peru/peruvian-vehicle` - duplicate from `vehicle-information.md`, hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/peru/peruvian-vehicle?fallback=true | ✅ | ✅ Same as above - query params handled by Docusaurus |
| https://docs.verifik.co/vehicle-validation/peru | ⏳ | |
| https://docs.verifik.co/vehicle-validation/paraguay | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/mexico | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/chile/vehicle-information?fallback=true | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/chile/driver-license | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/ecuador/vehicle-information-lookup | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/drivers-license | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/driver-s-license | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/drivers-license-verification | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/vehicle-values-by-code-fasecolda | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/vehicle-values-by-plate-fasecolda | ⏳ | Original file exists with correct slug |
| https://docs.verifik.co/vehicle-validation/colombia/vehicle-by-plate-and-identification-card-only?fallback=true | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/vehicle-s-soat-and-rtm-by-plate-and-identification-document | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/runt-vehicle-records-by-plate-number-simplified | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/runt-colombian-vehicle | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/simit-consultation | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/simit-resolutions | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/simit-resolutions?fallback=true | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/simit-fines.md | ⏳ | Original file exists (simit-fines.mdx) - may need .md redirect |
| https://docs.verifik.co/vehicle-validation/colombia/simit-agreements | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/general-query-of-simit-by-license-plate?fallback=true | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/general-query-of-simit-by-identification-document | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/sinister-verification-fasecolda | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/information-system-for-drivers-transporting-dangerous-goods?fallback=true | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/vehicle-fines-check-in-bogota-colombia | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/vehicle-fines-check-in-bogota-colombia?fallback=true | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/vehicle-fines-check-in-bogot-tta-colombia | ✅ | Duplicate file created - hidden from sidebar (typo preserved) |
| https://docs.verifik.co/vehicle-validation/colombia/colombian-vehicle-information-service | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/vehicle-validation/colombia/complete-vehicle-with-historical-records-by-license-plate-and-identification-card | ✅ | Duplicate file created - hidden from sidebar |
| https://docs.verifik.co/identity-validation | ✅ | Index file created with slug `/identity-validation` - hidden from sidebar |
| https://docs.verifik.co/identity-validation/united-states?fallback=true | ✅ | Index file created with slug `/identity-validation/united-states` - hidden from sidebar (query params handled automatically) |
| https://docs.verifik.co/identity-validation/united-states/ssn-verification-social-security-number | ✅ | Duplicate file created with slug `/identity-validation/united-states/ssn-verification-social-security-number` - hidden from sidebar |
| https://docs.verifik.co/identity-validation/united-states/ssn-verification-social-security-number?fallback=true | ✅ | Duplicate file created - query params handled automatically |
| https://docs.verifik.co/identity-validation/united-states/kansas-drivers-license-verification | ✅ | Duplicate file created with slug `/identity-validation/united-states/kansas-drivers-license-verification` - hidden from sidebar |
| https://docs.verifik.co/identity-validation/united-states/kansas-drivers-license-verification?fallback=true | ✅ | Duplicate file created - query params handled automatically |
| https://docs.verifik.co/identity-validation/united-states/florida-driver-license-validation?fallback=true | ✅ | Duplicate file created with slug `/identity-validation/united-states/florida-driver-license-validation` - hidden from sidebar (query params handled automatically) |
| https://docs.verifik.co/identity-validation/spain-citizen | ✅ | Duplicate file created with slug `/identity-validation/spain-citizen` - hidden from sidebar |
| https://docs.verifik.co/identity-validation/spain-citizen?fallback=true | ✅ | Duplicate file created - query params handled automatically |
| https://docs.verifik.co/identity-validation/peru/peru-citizen | ✅ | Duplicate file created with slug `/identity-validation/peru/peru-citizen` - hidden from sidebar |
| https://docs.verifik.co/identity-validation/peru/peruvian-foreigner-resident | ✅ | Duplicate file created with slug `/identity-validation/peru/peruvian-foreigner-resident` - hidden from sidebar |
| https://docs.verifik.co/identity-validation/peru?fallback=true | ✅ | Index file created with slug `/identity-validation/peru` - hidden from sidebar (query params handled automatically) |
| https://docs.verifik.co/identity-validation/peru/v3-peruvian-citizen | ✅ | Duplicate file created with slug `/identity-validation/peru/v3-peruvian-citizen` - hidden from sidebar |
| https://docs.verifik.co/identity-validation/uruguay-citizen | ✅ | Duplicate file created with slug `/identity-validation/uruguay-citizen` - hidden from sidebar |
| https://docs.verifik.co/identity-validation/guatemala?fallback=true | ✅ | Duplicate file created with slug `/identity-validation/guatemala` - hidden from sidebar (query params handled automatically) |
| https://docs.verifik.co/identity-validation/ecuador?fallback=true | ✅ | Duplicate file created with slug `/identity-validation/ecuador` - hidden from sidebar (query params handled automatically) |
| https://docs.verifik.co/identity-validation/ecuador | ✅ | Duplicate file created - same as above |
| https://docs.verifik.co/identity-validation/colombia/colombian-politically-exposed-persons | ✅ | New file created with slug `/identity-validation/colombia/colombian-politically-exposed-persons` - documents GET `/v2/co/politically-exposed-persons` endpoint - hidden from sidebar |
| https://docs.verifik.co/identity-validation/colombia/colombian-politically-exposed-persons?fallback=true | ✅ | Duplicate file created - query params handled automatically |
| https://docs.verifik.co/identity-validation/colombia/colombian-politically-exposed-persons.md | ✅ | .md version created with slug `/identity-validation/colombia/colombian-politically-exposed-persons` - hidden from sidebar |
| https://docs.verifik.co/identity-validation/colombia/special-stay-permit-pep | ✅ | Duplicate file created with slug `/identity-validation/colombia/special-stay-permit-pep` - hidden from sidebar |
| https://docs.verifik.co/identity-validation/colombia?fallback=true | ✅ | Index file created with slug `/identity-validation/colombia` - hidden from sidebar (query params handled automatically) |
| https://docs.verifik.co/identity-validation/chile/validate-chilean-document?fallback=true | ✅ | Duplicate file created with slug `/identity-validation/chile/validate-chilean-document` - hidden from sidebar (query params handled automatically) |
| https://docs.verifik.co/identity-validation/chile/chilean-citizen-information | ✅ | Duplicate file created with slug `/identity-validation/chile/chilean-citizen-information` - hidden from sidebar |
| https://docs.verifik.co/identity-validation/chile/chilean-citizen-information?fallback=true | ✅ | Duplicate file created - query params handled automatically |
| https://docs.verifik.co/identity-validation/chile/quebec-driver-license | ✅ | Redirect file created pointing to correct Canada location - hidden from sidebar |
| https://docs.verifik.co/identity-validation/brazil?fallback=true | ✅ | Duplicate file created with slug `/identity-validation/brazil` - hidden from sidebar (query params handled automatically) |
| https://docs.verifik.co/identity-validation/argentina | ✅ | Duplicate file created with slug `/identity-validation/argentina` - hidden from sidebar |
| https://docs.verifik.co/identity-validation/canada/quebec-driver-license | ✅ | File created with slug `/identity-validation/canada/quebec-driver-license` - notes that Quebec is not currently available, redirects to available provinces - hidden from sidebar |
| https://docs.verifik.co/background-check/colombia | ✅ | Index file created with slug `/background-check/colombia` - hidden from sidebar (query params handled automatically) |
| https://docs.verifik.co/background-check/colombia?fallback=true | ✅ | Index file created - query params handled automatically |
| https://docs.verifik.co/background-check/colombia.md | ✅ | .md version created with slug `/background-check/colombia` - hidden from sidebar |
| https://docs.verifik.co/background-check/colombia/verify-police-compliance-with-corrective-measures | ✅ | Duplicate file created with slug `/background-check/colombia/verify-police-compliance-with-corrective-measures` - hidden from sidebar |
| https://docs.verifik.co/background-check/colombia/verify-police-compliance-with-corrective-measures?fallback=true | ✅ | Duplicate file created - query params handled automatically |
| https://docs.verifik.co/background-check/colombia/verify-police-compliance-with-corrective-measures.md | ✅ | .md version created with slug `/background-check/colombia/verify-police-compliance-with-corrective-measures` - hidden from sidebar |
| https://docs.verifik.co/background-check/colombia-disciplinary-records-attorneys-office | ✅ | Slug added to existing file: `/background-check/colombia-disciplinary-records-attorneys-office` |
| https://docs.verifik.co/background-check/brazil.md | ✅ | .md version created with slug `/background-check/brazil` - hidden from sidebar |
| https://docs.verifik.co/business-validation/colombia | ✅ | Index file created with slug `/business-validation/colombia` - hidden from sidebar (query params handled automatically) |
| https://docs.verifik.co/business-validation/colombia?fallback=true | ✅ | Index file created - query params handled automatically |
| https://docs.verifik.co/business-validation/colombia/colombian-company-dian-verification | ✅ | Duplicate file created with slug `/business-validation/colombia/colombian-company-dian-verification` - hidden from sidebar |
| https://docs.verifik.co/business-validation/colombia/verify-legal-invoicer-in-dian-for-colombian-companies | ✅ | Duplicate file created with slug `/business-validation/colombia/verify-legal-invoicer-in-dian-for-colombian-companies` - hidden from sidebar |
| https://docs.verifik.co/business-validation/colombia/verify-legal-invoicer-in-dian-for-colombian-companies.md | ✅ | .md version created with slug `/business-validation/colombia/verify-legal-invoicer-in-dian-for-colombian-companies` - hidden from sidebar |
| https://docs.verifik.co/business-validation/colombia/verify-legal-invoicer-in-dian-for-colombian-companies?fallback=true | ✅ | Duplicate file created - query params handled automatically |
| https://docs.verifik.co/business-validation/colombia-rues-business-information | ✅ | Duplicate file created with slug `/business-validation/colombia-rues-business-information` - hidden from sidebar |
| https://docs.verifik.co/business-validation/colombia-rues-business-registry | ✅ | Duplicate file created with slug `/business-validation/colombia-rues-business-registry` - hidden from sidebar |
| https://docs.verifik.co/business-validation/mexican-companies | ✅ | Duplicate file created with slug `/business-validation/mexican-companies` - hidden from sidebar |
| https://docs.verifik.co/api/projects/projects-update | ✅ | Redirect file created with slug `/api/projects/projects-update` pointing to `/resources/projects/update-a-project` - hidden from sidebar |
| https://docs.verifik.co/api/projects/projects-update/ | ✅ | Redirect file created - trailing slash handled automatically |
| https://docs.verifik.co/api/projects/projects-create | ✅ | Duplicate file created with slug `/api/projects/projects-create` - hidden from sidebar |
| https://docs.verifik.co/v2/co/fasecolda/values-by-code?codeFasecolda= | ✅ | API endpoint - correct as is (not a documentation URL) |
| https://docs.verifik.co/v2/app-logins/66e464acbad79f3a380d408f?populates[]=biometricValidation | ✅ | API endpoint with ID - correct as is (not a documentation URL) |
| https://docs.verifik.co/v2/co/bogota/vehicle/fines?plate= | ✅ | API endpoint - correct as is (not a documentation URL) |
| https://docs.verifik.co/docs/intro | ✅ | Slug added to intro.md: `/docs/intro` |
| https://docs.verifik.co/docs/ocr-reconocimiento-optico-de-caracteres/9orqb045fe6n7-scan-studio | ✅ | Old path structure - redirects to `/ocr-engines/scan-studio` (handled by Docusaurus routing) |
| https://docs.verifik.co/examples | ✅ | Index file created with slug `/examples` - hidden from sidebar |
| https://docs.verifik.co/military | ✅ | Placeholder file created with slug `/military` noting service not available - hidden from sidebar |
| https://docs.verifik.co/identity/bolivia | ✅ | File already exists with slug `/identity/bolivia` |
| https://docs.verifik.co/identity/guatemala | ✅ | File already exists with slug `/identity/guatemala` |
| https://docs.verifik.co/biometrics/compare | ✅ | Slug added to compare.mdx: `/biometrics/compare` |
| https://docs.verifik.co/scandocs/scan-prompt | ✅ | Redirect file created with slug `/scandocs/scan-prompt` pointing to `/ocr-engines/scan-prompt` - hidden from sidebar |
| https://docs.verifik.co/blog/rss.xml | ✅ | RSS feed - correct as is (handled by Docusaurus blog plugin) |

### 11. Verifik-es URLs with ?fallback=true (Redirects - May need proper handling)

| URL | Status | Notes |
|-----|--------|-------|
| https://docs.verifik.co/verifik-es/identity-validation/colombia?fallback=true | ✅ | File exists with slug `/identity-validation/colombia` - query params handled automatically |
| https://docs.verifik.co/verifik-es/identity-validation/ecuador?fallback=true | ✅ | File exists with slug `/identity-validation/ecuador` - query params handled automatically |
| https://docs.verifik.co/verifik-es/identity-validation/peru/peruvian-citizen?fallback=true | ✅ | Duplicate file created with slug `/identity-validation/peru/peruvian-citizen` - hidden from sidebar (query params handled automatically) |
| https://docs.verifik.co/verifik-es/identity-validation/colombia/colombian-politically-exposed-persons?fallback=true | ✅ | New file created with slug `/identity-validation/colombia/colombian-politically-exposed-persons` - hidden from sidebar (query params handled automatically) |
| https://docs.verifik.co/verifik-es/identity-validation/united-states/ssn-verification-social-security-number | ✅ | File exists with slug `/identity-validation/united-states/ssn-verification-social-security-number` |
| https://docs.verifik.co/verifik-es/vehicle-validation/colombia/verifik-pico-y-placa-for-bogota?fallback=true | ✅ | Duplicate file created with slug `/vehicle-validation/colombia/verifik-pico-y-placa-for-bogota` - hidden from sidebar (query params handled automatically) |
| https://docs.verifik.co/verifik-es/vehicle-validation/colombia/vehicle-fines-check-in-bogota-colombia?fallback=true | ✅ | Duplicate file created with slug `/vehicle-validation/colombia/vehicle-fines-check-in-bogota-colombia` - hidden from sidebar (query params handled automatically) |
| https://docs.verifik.co/verifik-es/health/verify-colombian-affiliations?fallback=true | ✅ | Duplicate file created with slug `/health/verify-colombian-affiliations` - hidden from sidebar (query params handled automatically) |
| https://docs.verifik.co/verifik-es/health/verify-colombian-affiliations | ✅ | Duplicate file created - query params handled automatically |
| https://docs.verifik.co/verifik-es/health | ✅ | Index file created with slug `/health` - hidden from sidebar |
| https://docs.verifik.co/verifik-es/accounts/authentication/renew-your-token-jwt?fallback=true | ✅ | File exists with slug `/accounts/authentication/renew-your-token-jwt` - query params handled automatically |
| https://docs.verifik.co/verifik-es/services/verifik-access-code-solution-via-api?fallback=true | ✅ | Index file created with slug `/services/verifik-access-code-solution-via-api` - hidden from sidebar (query params handled automatically) |
| https://docs.verifik.co/verifik-es/services/verifik-enroll-code-solution-via-api?fallback=true | ✅ | Index file created with slug `/services/verifik-enroll-code-solution-via-api` - hidden from sidebar (query params handled automatically) |
| https://docs.verifik.co/verifik-es/biometrics-apis/comparar/comparar-1-1?fallback=true | ✅ | New file created with slug `/biometrics-apis/comparar/comparar-1-1` - hidden from sidebar (query params handled automatically) |
| https://docs.verifik.co/verifik-es/?fallback=true | ✅ | File exists (intro.md) - query params handled automatically |
| https://docs.verifik.co/verifik-es/resources/persons?fallback=true | ✅ | Slug updated to `/resources/persons` in personas.mdx - query params handled automatically |

### 12. Other Issues

| URL | Status | Notes |
|-----|--------|-------|
| https://docs.verifik.co/verifik-es/certificados/sena-certificados.md | ⏳ | .md extension |
| https://docs.verifik.co/verifik-es/certificates/registry-certificate | ⏳ | English path in verifik-es |
| https://docs.verifik.co/verifik-es/cuentas/autentication/renovar-tu-token-jwt | ⏳ | Typo: autentication |
| https://docs.verifik.co/verifik-es/cuentas/autenticacion/telefono-de-autenticacion | ⏳ | |
| https://docs.verifik.co/verifik-es/cuentas/autenticacion/renueva-tu-token-jwt | ⏳ | |
| https://docs.verifik.co/verifik-es/cuentas/autenticacion/renueva-tu-token-jwt-cuentas | ⏳ | |
| https://docs.verifik.co/verifik-es/legal/consulta-de-registros-de-casos-judiciales | ⏳ | |
| https://docs.verifik.co/verifik-es/legal/procesos-judiciales-colombianos.md | ⏳ | .md extension |
| https://docs.verifik.co/verifik-es/legal/court-case-records-lookup | ⏳ | English path |
| https://docs.verifik.co/verifik-es/legal/obtener-detalles-de-un-proceso-legal-por-numero | ⏳ | |
| https://docs.verifik.co/verifik-es/legal/colombian-legal-processes | ⏳ | English path |
| https://docs.verifik.co/verifik-es/militar | ⏳ | |
| https://docs.verifik.co/verifik-es/finanzas | ⏳ | |
| https://docs.verifik.co/verifik-es/finance/deudores-morosos | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/servicios | ⏳ | |
| https://docs.verifik.co/verifik-es/servicios/verifik-access-code-solution-via-api.md | ⏳ | .md extension |
| https://docs.verifik.co/verifik-es/servicios/verifik-access-code-solution-via-api/ejemplo-de-acceso-biometrico | ⏳ | |
| https://docs.verifik.co/verifik-es/recursos/personas/recuperar-una-persona.md | ⏳ | .md extension |
| https://docs.verifik.co/verifik-es/recursos/personas | ⏳ | |
| https://docs.verifik.co/verifik-es/recursos/flujos-de-proyecto | ⏳ | |
| https://docs.verifik.co/verifik-es/recursos/flujos-de-proyecto/list-all-project-flows-listar-todos-los-flujos-de-proyecto | ⏳ | |
| https://docs.verifik.co/verifik-es/recursos/flujos-de-proyecto/the-projectflow-object-objetos-de-flujo-de-proyecto | ⏳ | |
| https://docs.verifik.co/verifik-es/recursos/webhooks/eliminar-un-webhook | ⏳ | |
| https://docs.verifik.co/verifik-es/recursos/webhooks/listar-todos-los-webhooks | ⏳ | |
| https://docs.verifik.co/verifik-es/recursos/webhooks/el-objeto-de-webhook | ⏳ | |
| https://docs.verifik.co/verifik-es/recursos/validaciones-biometricas | ⏳ | |
| https://docs.verifik.co/verifik-es/recursos/validaciones-biometricas/recuperar-una-validacion-biometrica.md | ⏳ | .md extension |
| https://docs.verifik.co/verifik-es/recursos/validaciones-biometricas/crear-validacion-biometrica | ⏳ | |
| https://docs.verifik.co/verifik-es/recursos/validaciones-de-telefono/crear-validacion-de-telefono | ⏳ | |
| https://docs.verifik.co/verifik-es/recursos/validaciones-de-correo-electronico | ⏳ | |
| https://docs.verifik.co/verifik-es/recursos/validaciones-de-correo-electronico/crear-validacion-de-correo-electronico | ⏳ | |
| https://docs.verifik.co/verifik-es/recursos/documentos-de-validacion/el-objeto-de-validacion-de-documentos | ⏳ | |
| https://docs.verifik.co/verifik-es/recursos/inicios-de-sesion-de-app/listar-los-inicios-de-sesion-en-la-app | ⏳ | |
| https://docs.verifik.co/verifik-es/recursos/inicios-de-sesion-de-app/recuperar-inicio-de-sesion-en-la-app | ⏳ | |
| https://docs.verifik.co/verifik-es/recursos/registros-en-la-app | ⏳ | |
| https://docs.verifik.co/verifik-es/recursos/registros-en-la-app/registrar-objetos-en-la-app | ⏳ | |
| https://docs.verifik.co/verifik-es/api-s-de-biometria/busqueda/busqueda-1-n.md | ⏳ | .md extension |
| https://docs.verifik.co/verifik-es/api-s-de-biometria/busqueda/buscar-1-n-or-crop-recortar | ⏳ | |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/colombia/permiso-especial-de-permanencia-pep-documento-de-identificacion | ⏳ | |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/colombia/cedula-de-extranjeria-colombiana-ce | ⏳ | |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/venezuela/informacion-del-ciudadano-venezolano | ⏳ | |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/venezuela/ciudadanos-extranjeros-en-venezuela.md | ⏳ | .md extension |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/honduras.md | ⏳ | .md extension |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/brasil.md | ⏳ | .md extension |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/paraguay | ⏳ | |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/bolivia | ⏳ | |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/argentina | ⏳ | |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/el-salvador | ⏳ | |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/costa-rica | ⏳ | |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/mexico | ⏳ | |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/estados-unidos | ⏳ | |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/estados-unidos/validacion-de-licencia-de-conducir-de-florida | ⏳ | |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/canada/licencia-de-conducir-de-ontario | ⏳ | |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/peru/ciudadano-peruano | ⏳ | |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/ciudadano-espanol | ⏳ | |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/chile | ⏳ | |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/uruguay | ⏳ | |
| https://docs.verifik.co/verifik-es/validaciones-de-identidad/venezuela/venezuelan-foreigner-citizens | ⏳ | English path |
| https://docs.verifik.co/verifik-es/validacion-empresarial/colombia/complete-business-registry-rues | ⏳ | English path |
| https://docs.verifik.co/verifik-es/business-validation/brazil | ⏳ | English path |
| https://docs.verifik.co/verifik-es/business-validation/colombia/complete-business-registry-rues | ⏳ | English path |
| https://docs.verifik.co/verifik-es/vehicle-validation | ⏳ | English path |
| https://docs.verifik.co/verifik-es/vehicle-validation/argentina | ⏳ | English path |
| https://docs.verifik.co/verifik-es/vehicle-validation/chile/driver-license | ⏳ | English path |
| https://docs.verifik.co/verifik-es/vehicle-validation/chile/vehicle-information | ⏳ | English path |
| https://docs.verifik.co/verifik-es/vehicle-validation/chile/licencia-de-conducir | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/vehicle-validation/colombia/taxes-in-bogota | ⏳ | English path |
| https://docs.verifik.co/verifik-es/vehicle-validation/colombia/vehiculo-por-codigo-fasecolda | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/vehicle-validation/colombia/vehicle-values-by-plate-fasecolda | ⏳ | English path |
| https://docs.verifik.co/verifik-es/vehicle-validation/colombia/verifik-pico-y-placa-for-bogota | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/vehicle-validation/ecuador/vehicle-information-lookup | ⏳ | English path |
| https://docs.verifik.co/verifik-es/vehicle-validation/peru/peruvian-vehicle-insurance | ⏳ | English path |
| https://docs.verifik.co/verifik-es/identity-validation/canada/ontario-driver-license | ⏳ | English path |
| https://docs.verifik.co/verifik-es/identity-validation/chile/chilean-taxpayer-information | ⏳ | English path |
| https://docs.verifik.co/verifik-es/identity-validation/colombia/colombian-citizen | ⏳ | English path |
| https://docs.verifik.co/verifik-es/identity-validation/colombia/colombian-politically-exposed-persons | ⏳ | English path |
| https://docs.verifik.co/verifik-es/identity-validation/colombia | ⏳ | English path |
| https://docs.verifik.co/verifik-es/identity-validation/ecuador | ⏳ | English path |
| https://docs.verifik.co/verifik-es/identity-validation/guatemala | ⏳ | English path |
| https://docs.verifik.co/verifik-es/identity-validation/mexican | ⏳ | English path |
| https://docs.verifik.co/verifik-es/identity-validation/peru/peruvian-foreigner-resident | ⏳ | English path |
| https://docs.verifik.co/verifik-es/identity-validation/spain-citizen | ⏳ | English path |
| https://docs.verifik.co/verifik-es/identity-validation/united-states/ssn-verification-social-security-number | ⏳ | English path |
| https://docs.verifik.co/verifik-es/identity-validation/uruguay | ⏳ | English path |
| https://docs.verifik.co/verifik-es/identity-validation/canada | ⏳ | English path |
| https://docs.verifik.co/verifik-es/identity-validation/costa-rica | ⏳ | English path |
| https://docs.verifik.co/verifik-es/verificacion-de-antecedentes/internacional/verificacion-de-antecedentes-de-interpol | ⏳ | |
| https://docs.verifik.co/verifik-es/verificacion-de-antecedentes/internacional/verificacion-de-antecedentes-de-la-dea | ⏳ | |
| https://docs.verifik.co/verifik-es/verificacion-de-antecedentes/internacional/verificacion-de-antecedentes-de-la-onu | ⏳ | |
| https://docs.verifik.co/verifik-es/verificacion-de-antecedentes/colombia/verificacion-de-antecedentes-policiales-en-colombia | ⏳ | |
| https://docs.verifik.co/verifik-es/background-check/colombia/verify-police-compliance-with-corrective-measures | ⏳ | English path |
| https://docs.verifik.co/verifik-es/apis-de-escaneo-de-documnetos/scan-studio/documentos-permitidos | ⏳ | Typo: documnetos |
| https://docs.verifik.co/verifik-es/planes-y-precios/precios-de-sms-whatsapp | ⏳ | |
| https://docs.verifik.co/verifik-es/planes-y-precios/precios-de-sms-whatsapp/ | ⏳ | |
| https://docs.verifik.co/verifik-es/planes-y-precios/planes-de-smart-access/planes-empresariales | ⏳ | |
| https://docs.verifik.co/verifik-es/planes-y-precios/planes-de-smart-access/plan-gratuito | ⏳ | |
| https://docs.verifik.co/verifik-es/planes-y-precios/planes-de-smart-access/sesiones | ⏳ | |
| https://docs.verifik.co/verifik-es/planes-y-precios/planes-de-smart-access/sesiones-smart-access | ⏳ | |
| https://docs.verifik.co/verifik-es/plans-and-pricings/planes-de-smart-access | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/plans-and-pricings/planes-de-smart-access/usuarios-activos | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/plans-and-pricings/planes-de-smart-access/usuarios-activos-smart-access | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/plans-and-pricings/planes-de-smartenroll | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/plans-and-pricings/smart-access-plans | ⏳ | English path |
| https://docs.verifik.co/verifik-es/plans-and-pricings/smart-access-plans/free-plans | ⏳ | English path |
| https://docs.verifik.co/verifik-es/plans-and-pricings/smart-access-plans-planes-de-acceso-inteligente/sessions-sesiones | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/plans-and-pricings/smart-access-plans-planes-de-acceso-inteligente/sessions-sesiones-smart-access | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/plans-and-pricings/planes-de-smart-access-plans-and-pricings | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/proyectos/listar-todos-los-proyectos | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/proyectos/crear-un-proyecto | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/flujos-de-proyecto/create-a-project-flow-crear-un-flujo-de-proyecto/security-settings-configuracion-de-seguridad | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/flujos-de-proyecto/create-a-project-flow-crear-un-flujo-de-proyecto/login-settings-configuracion-de-inicio-de-sesion | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/project-flows | ⏳ | English path |
| https://docs.verifik.co/verifik-es/resources/project-flows/retrieve-a-project-flow | ⏳ | English path |
| https://docs.verifik.co/verifik-es/resources/project-flows/create-a-project-flow | ⏳ | English path |
| https://docs.verifik.co/verifik-es/resources/project-flows/create-a-project-flow-crear-un-flujo-de-proyecto | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/project-flows/the-projectflow-object | ⏳ | English path |
| https://docs.verifik.co/verifik-es/resources/project-flows/create-a-project-flow-crear-un-flujo-de-proyecto | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/persons-personas/list-all-persons-listar-todas-las-personas | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/persons/update-a-person | ⏳ | English path |
| https://docs.verifik.co/verifik-es/resources/persons?fallback=true | ⏳ | English path |
| https://docs.verifik.co/verifik-es/resources/personas/crear-una-persona-con-deteccion-de-vida | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/phone-validations/retrieve-a-phone-validation | ⏳ | English path |
| https://docs.verifik.co/verifik-es/resources/phone-validations/create-phone-validation | ⏳ | English path |
| https://docs.verifik.co/verifik-es/resources/app-logins-inicios-de-sesion-de-app/retrieve-an-app-login-recuperar-inicio-de-sesion-en-la-app | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/email-validations-validaciones-de-correo-electronico/retrieve-an-email-validation-recuperar-una-validacion-de-correo-electronico | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/validaciones-de-correo-electronico/lista-de-validaciones-de-correo-electronico | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/webhooks/delete-a-webhook | ⏳ | English path |
| https://docs.verifik.co/verifik-es/resources/webhooks/delete-a-webhook-eliminar-un-webhook | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/webhooks/create-a-webhook-crear-un-webhook | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/webhooks/update-a-webhook | ⏳ | English path |
| https://docs.verifik.co/verifik-es/resources/webhooks/update-a-webhook-actualizar-un-webhook | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/webhooks/list-all-webhooks | ⏳ | English path |
| https://docs.verifik.co/verifik-es/resources/webhooks/retrieve-a-webhook | ⏳ | English path |
| https://docs.verifik.co/verifik-es/resources/validaciones-biometricas/recuperar-una-validacion-biometrica | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/validaciones-de-telefono/el-objeto-de-validaciones-de-telefono | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/flujos-de-proyecto/create-a-project-flow-crear-un-flujo-de-proyecto | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/flujos-de-proyecto/create-a-project-flow-crear-un-flujo-de-proyecto-flujos | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/flujos-de-proyecto/create-a-project-flow-crear-un-flujo-de-proyecto/login-settings-configuracion-de-inicio-de-sesion | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/flujos-de-proyecto/create-a-project-flow-crear-un-flujo-de-proyecto/security-settings-configuracion-de-seguridad | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/project-flows-flujos-de-proyecto/create-a-project-flow-crear-un-flujo-de-proyecto | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/app-logins-inicios-de-sesion-de-app | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/resources/registros-en-la-app | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/services/verifik-access-code-solution-via-api/ejemplo-acceso-telefonico | ⏳ | |
| https://docs.verifik.co/verifik-es/services/verifik-access-code-solution-via-api/biometric-access-example-ejemplo-de-acceso-biometrico | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/services/verifik-access-code-solution-via-api/ejemplo-de-acceso-biometrico | ⏳ | |
| https://docs.verifik.co/verifik-es/services/verifik-enroll-code-solution-via-api/tutorial | ⏳ | |
| https://docs.verifik.co/verifik-es/services/registro-inteligente | ⏳ | |
| https://docs.verifik.co/verifik-es/services/acceso-inteligente | ⏳ | |
| https://docs.verifik.co/verifik-es/services/verifik-database-screening | ⏳ | English path |
| https://docs.verifik.co/verifik-es/services/verifik-database-screening/app-query-example | ⏳ | English path |
| https://docs.verifik.co/verifik-es/services/verifik-database-screening/app-query-example-ejemplo-de-consulta-en-la-aplicacion | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/services/verifik-database-screening/ejemplo-de-consulta-en-la-aplicacion | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/biometrics-apis/busqueda | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/biometrics-apis/compare/compare-1-1 | ⏳ | English path |
| https://docs.verifik.co/verifik-es/biometrics-apis/compare/compare-and-liveness-detection | ⏳ | English path |
| https://docs.verifik.co/verifik-es/biometrics-apis/busqueda/buscar-1-n-or-live-vida | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/scan-docs-apis/scan-studio/example-tarjeta-de-propiedad | ⏳ | Mixed language |
| https://docs.verifik.co/verifik-es/scan-docs-apis/scan-studio/documentos-permitidos | ⏳ | |

## Summary

- **Total URLs to review**: ~500
- **High Priority (in codebase)**: ~20 URLs
- **Medium Priority (redirects/external)**: ~100 URLs  
- **Low Priority (may be intentional)**: ~380 URLs

## Next Steps

1. Fix `/validacion-de-vehiculo/` typo in source files (HIGH PRIORITY)
2. Fix `/docs-es/` prefix issues (HIGH PRIORITY)
3. Fix `/background-check/international/` paths (HIGH PRIORITY)
4. Review and fix trailing dot issues
5. Handle GitBook redirects (may need server config)
6. Review root-level English paths (may be intentional)
7. Review URLs with `?fallback=true` (may need redirect rules)
