...
title: 哥伦比亚 — 检察院（Procuraduría）纪律记录
description: 哥伦比亚检察院（Procuraduría）出具的纪律处分记录核验。
...
| 名称              | 类型    | 必填 | 描述                                                       |
| ----------------- | ------- | ---- | ---------------------------------------------------------- |
| `documentType`    | string  | 是   | 证件类型。允许取值：CC、PEP、CE。                          |
...
<Tabs>
<TabItem value="200-with-records" label="200（存在记录）">
...
<TabItem value="409-2" label="409（服务端错误）">
