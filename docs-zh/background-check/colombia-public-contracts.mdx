...
title: 哥伦比亚 - 公共合同
description: 查询与个人或公司关联的哥伦比亚公共合同列表。
slug: /zh-docs/background-check/colombia-public-contracts
---
...
根据证件类型与号码获取与国家签订的公共合同列表，可包含承包商、金额、机构、期限等信息。
...
| 名称             | 类型   | 必填 | 说明 |
| ---------------- | ------ | ---- | ---- |
| `documentType`   | string | 是   | CC 或 NIT。 |
| `documentNumber` | string | 是   | 要查询的号码。 |
