...
| 托管 SmartEnroll / project flow 允许范围 | **`0.65`–`0.95`** |
| 直接 face-recognition API（`compare_min_score`） | **`0.67`–`0.95`**（省略时默认 `0.85`） |

印刷证件肖像常常褪色、有划痕或分辨率很低。部分哥伦比亚身份证（cédula）质量尤其差，其他国家也可能出现同样的证件质量问题。活体自拍对比印刷照片的分数通常**低于**活体对活体。将项目阈值降至 **`0.65`** 可减少证件质量差的真实用户被误拒，但也会提高误通过风险。除非看到真实用户失败，否则请保持 `0.85`。
