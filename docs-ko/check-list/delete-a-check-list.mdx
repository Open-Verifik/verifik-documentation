...
title: 체크리스트 삭제
description: 소유한 Check List를 삭제하고 제거된 문서를 받습니다
...
소유한 Check List를 삭제합니다. API는 삭제된 문서를 `{ data }`로 반환합니다. 잘못된 ObjectId, 없는 목록, 다른 클라이언트가 소유한 목록은 `404`를 반환합니다.
...
클라이언트의 컬렉션에서 목록을 제거하고 삭제된 문서를 반환합니다. 이 호출 이후 같은 id에 대한 GET / PUT / DELETE는 `404`를 반환합니다.
