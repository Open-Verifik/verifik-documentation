...
인쇄된 신분증 초상은 바랜 잉크, 긁힘, 저해상도인 경우가 많습니다. 일부 콜롬비아 신분증(cédula)은 특히 품질이 낮고, 같은 문제가 다른 국가에서도 발생할 수 있습니다. 라이브 셀피 vs 인쇄 사진은 라이브 vs 라이브보다 **낮은** 점수가 나옵니다. 프로젝트 임계값을 **`0.65`**까지 낮추면 품질이 나쁜 문서를 가진 정상 사용자의 오거부가 줄지만 오수락 위험은 커집니다. 실제 사용자가 실패하지 않으면 `0.85`를 유지하세요.
