...
**대상:** 공식 콜롬비아 출처에서 이름을 조회할 수 있는 문서. `/v2/co/cedula`는 Procuraduría, DIAN, ADRES, Registraduría를 통합합니다. **CE**, **PEP**, **NIT**는 보통 Procuraduría로 해석됩니다.

| 유형 | 보유자 | `documentType` | 콜롬비아 일반 길이 | 이 API 허용 |
| --- | --- | --- | --- | --- |
| **CC** | 콜롬비아 국민 | `CC` | **3–10**자리 (흔함: **8** 또는 **10** NUIP) | **5–10**자리, 숫자만 |
| **CE** | 외국인 거주자 (이름 조회) | `CE` | 보통 **6–7**자리 | **5–10**자리, 숫자만 |
| **PPT** | 베네수엘라 이주민 (이 경로 이름 조회) | `PPT` | 최대 **7**자리 | **5–10**자리, 숫자만 |
| **PEP** | Permiso Especial de Permanencia (이름 조회) | `PEP` | 이 경로에서 보통 **7–10**자리 | **5–10**자리, 숫자만 |
| **NIT** | 세금 ID | `NIT` | **9–10**자리 | **5–10**자리, 숫자만 |

**`documentNumber` 입력:** 숫자만 — 점, 공백, 하이픈 없음. CC 예: `1032386359`.

**다른 엔드포인트 사용:**
- **Procuraduría 이름만** → [콜롬비아 Procuraduría](/verifik-ko/identity/colombia-procuraduria) (`/v2/co/procuraduria`)
- **CE 이민 상태** (만료) → [콜롬비아 CE](/verifik-ko/identity/colombia-ce) (`/v2/co/foreigner-id/ce` + `expeditionDate`)
...
| 이름 | 형식 | 필수 | 설명 |
| --- | --- | --- | --- |
| `documentType` | string | 예 | **`CC`**, **`CE`**, **`PPT`**, **`NIT`**, **`PEP`** 중 하나. [신분증 가이드](/verifik-ko/identity-validation/colombia/colombia-identity-documents-guide) 참고. |
...
- **`documentType`**은 **`CC`**, **`CE`**, **`PPT`**, **`NIT`**, **`PEP`**입니다. 이름만 Procuraduría에서 조회하려면 [콜롬비아 Procuraduría](/verifik-ko/identity/colombia-procuraduria)를 사용하세요.
