...
이 서비스를 통해 콜롬비아에서 특정 차량 번호판과 연결된 과태료 및 단속에 대한 상세 정보를 얻을 수 있습니다. 응답에는 각 단속의 위반 유형, 과태료 금액, 현재 상태가 포함됩니다.
...
| 이름 | 유형 | 필수 | 설명 |
| --- | --- | --- | --- |
| `plate` | string | 예 | 조회할 번호판(공백·마침표 없음). |
...
- 이 종합 서비스는 과태료, 교육 과정, 분할 납부 합의를 포함하여 차량에 대한 모든 SIMIT 데이터를 제공합니다.
- « Multas »에는 차량 번호판과 관련된 교통 위반 세부 정보가 있습니다.
- « Cursos »에는 차량 위반과 관련된 교통 안전 교육 과정이 표시됩니다.
