...
description: 콜롬비아 SIMIT 시스템의 교통 위반 분할 납부 합의 조회 서비스.
...
SIMIT 분할 납부 합의 서비스는 콜롬비아 SIMIT 시스템에서 교통 위반과 관련된 분할 납부 합의를 효율적으로 조회할 수 있는 방법을 제공합니다. 관련 정보를 입력해 조회하면 위반자 상세, 결의 번호, 납부 금액 등 분할 납부 합의에 대한 종합 정보를 얻을 수 있습니다.
...
- SIMIT 분할 납부 합의는 위반자가 교통 위반 과태료를 분할 납부할 수 있도록 합니다.
- "Acuerdo de pago" 상태는 활성 분할 납부 합의를 의미합니다.
- `permitePago: "N"` 은 현재 납부가 허용되지 않음을 의미합니다.
- 금액은 콜롬비아 페소(COP)로 제공됩니다.
- 결의 번호는 특정 합의를 추적하는 데 사용됩니다.
