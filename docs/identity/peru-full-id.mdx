...
Returns **extended RENIEC DNI data** for Peruvian citizens: full name, civil status, dates, **photo (base64)**, address, ubigeo, and verification digit. Send `documentType=DNI` and the 8-digit `documentNumber`. Optional `force=1` asks for a fresh lookup when your integration allows it (see request examples).
...
| `documentNumber` | string | **Yes** | Eight-digit DNI number (no spaces). | `12345678` |
| `force` | string | No | Optional. Pass `1` to request a refreshed record when supported (see examples using `force=1`). | `1` |
