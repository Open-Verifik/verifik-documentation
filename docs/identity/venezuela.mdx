...
| -------------- | ------- | -------- | ---------------------------------------------- |
| `documentNumber` | string | Yes      | National cédula number (CCVE). Dots and spaces are stripped. Do not send `documentType`—this route always uses **CCVE**. |
...
- When present, `birthDate` follows `YYYY-MM-DD` (e.g., "1998-03-01").
