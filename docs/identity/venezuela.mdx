---
id: venezuela
title: Venezuelan Citizen (CCVE)
description: National identity lookup for Venezuela—verify cédula (CCVE) holders by document number via GET /v2/ve/cedula for KYC and fraud prevention.
...
export const structuredData = {
	...
	"@type": "TechArticle",
	headline: "Venezuela national ID (CCVE) verification | Verifik KYC API",
	description:
		"National identity lookup for Venezuelan citizens: validate cédula (CCVE) data by document number using Verifik's GET /v2/ve/cedula endpoint.",
	articleSection: "API Documentation",
	keywords: "Venezuela CCVE, cédula nacional, identity verification, KYC API, v2/ve/cedula, fraud prevention",
	about: {
		"@type": "Thing",
...
This route returns identity data for **Venezuelan nationals** using the **CCVE** (*Cédula de Identidad*) document class. Send the cédula number as **`documentNumber`**; the API resolves the record and returns structured name fields for **KYC**, onboarding, and fraud checks. For **foreign residents**, use [`GET /v2/ve/foreigner-id`](/identity/venezuela-foreigner-id) (CEVE) instead.
...
| -------------- | ------- | -------- | ---------------------------------------------- |
| `documentNumber` | string | Yes      | National cédula number (CCVE). No `documentType` query is required—the service applies **CCVE** for this endpoint. |
