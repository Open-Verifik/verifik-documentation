...
description: Verify Panamanian CCPA national ID data (document, name, date of birth) against official civil-registry sources for KYC and fraud prevention.
...
export const structuredData = {
	...
	description:
		"Verify Panamanian CCPA national ID data (document, name, date of birth) against official civil-registry sources for KYC and fraud prevention.",
...
Verify Panamanian **CCPA** (*Cédula de Identidad Personal*) data against official civil-registry sources. Send **`documentType`** (`CCPA`), a normalized **`documentNumber`**, and **`dateOfBirth`** in **`DD/MM/YYYY`**; the response confirms that the identity record is consistent for **KYC**, customer onboarding, and fraud controls.
...
5. **Pre–2006 civil registry (AV)** — Province immediately followed by `AV`. Example: `10AV-1234-12345`
6. **Indigenous (PI)** — Province immediately followed by `PI`. Example: `1PI-1234-12345`

Official province codes are **1–13**. Invalid codes (e.g. `14`) are rejected.
...
Format rules align with the open [cedula-panama](https://github.com/merlos/cedula-panama) validator (MIT).
