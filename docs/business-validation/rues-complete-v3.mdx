---
id: rues-complete-v3
title: RUES Complete Business Validation (v3)
description: Retrieve complete Colombian business information from RUES with Verifik's KYB API. Connect with official sources and verify comprehensive business data.
...
export const structuredData = {
	...
	"@type": "TechArticle",
	headline: "RUES Complete Business Validation (v3) | Verifik KYB API",
	description:
		"Retrieve complete Colombian business information from RUES with Verifik's KYB API. Connect with official sources and verify comprehensive business data.",
	articleSection: "API Documentation",
	keywords: "business verification, KYB API, Colombia RUES validation, NIT validation, company verification, official data source, fraud prevention, commercial registry",
	about: {
		"@type": "Thing",
...
-   **Multiple categories**: Same registry lines as **`/v3/co/rues`** — e.g. **RM** (Registro Mercantil), **PROP**, **RUNEOL**, **RNT**, **ESAL**, **ESOL**, **JUEGOS**, **EXTRANJERAS** (see parameters below).
...
Retrieves **complete** Colombian business information from the RUES registry (same query parameters as `GET /v3/co/rues`, with a much richer response).

## Which endpoint should I use?

| | **`GET /v3/co/rues`** (basic) | **`GET /v3/co/rues-complete`** (this API) |
| --- | --- | --- |
| **Purpose** | Fast KYB check; category-specific **summary** | Full **dossier** for onboarding or enhanced due diligence |
| **Typical fields** | Core registry fields for the chosen category (e.g. RM: name, NIT, chamber, matrícula status) | Commercial registry, **economic activities**, **establishments**, **legal representatives**, and related blocks |
| **Parameters** | `documentType` (**`NIT`**), `documentNumber`, optional `category` | **Same** |
| **Catalog pricing** | Lower (e.g. **0.2** credits / **0.3** SmartCheck) | Higher (e.g. **0.3** credits / **0.4** SmartCheck) because more upstream data is aggregated |

Use the **basic** endpoint when you only need to confirm registration and key identifiers. Use **complete** when you need representatives, activities, establishments, or a signed full record for compliance workflows.

See also: [Colombia — RUES business lookup (v3)](/business-validation/colombia-rues-v3).
...
Also available at `GET https://verifik.app/v3/co/rues-complete` (same path on the app host).
...
| `documentNumber` | string | Yes | Tax ID; non-alphanumeric characters are stripped server-side. |
| `category` | string | No | Registry line: **`RM`**, **`PROP`**, **`RUNEOL`**, **`RNT`**, **`ESAL`**, **`ESOL`**, **`JUEGOS`**, **`EXTRANJERAS`**. Defaults to **`RM`** if omitted. |

### Notes

- This route accepts **only** the parameters above. **`municipality`**, **`chamber`**, and **`page`** are **not** supported on **`GET /v3/co/rues-complete`**.
- For a **lighter** summary (same query shape), use **`GET /v3/co/rues`** ([Colombia RUES business lookup (v3)](/business-validation/colombia-rues-v3)).
- **Pricing:** the complete pull costs more than the basic lookup because Verifik aggregates additional RUES detail (representatives, activities, establishments, etc.) in one signed response.
