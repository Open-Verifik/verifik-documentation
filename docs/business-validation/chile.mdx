...
The HTTP reference for Chile business validation lives in **[Chile — Business Validation](/business-validation/chilean)**. That page is the canonical definition for `AppFeature.docs` seeding so this marketing overview does not duplicate the same endpoint.
