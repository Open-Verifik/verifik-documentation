...
:::warning
The JWT Token you should use when validating `App Registration Phone Validations` is provided from the `App Registration` in creation. You must use the token returned when creating an [App Registration](create-an-app-registration) to authenticate this request.
:::
