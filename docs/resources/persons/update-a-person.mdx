...
| `images` | string[] | No | 1–3 raw base64 face images. **Replaces** every enrolled face with this set (does not append). |
...
- If you send **`images`**, use 1–3 raw base64 strings. That set **replaces** the enrolled faces; it does not add on top of the old crop. Tiny payloads may return **412** `only_images_in_base64`. To keep an old pose and add a new one, send both images in the same request.
