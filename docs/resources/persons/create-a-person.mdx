...
You can also find the same record with [Lookup a Person by Name](/resources/lookup-a-person). Very short image payloads may still return `412` with `"message": "only_images_in_base64"`.
