...
| `allowedCountries` | array | Yes | Countries allowed to enroll. Use country names (for example `"United States"`) or `"All"` for every country. Document acceptance is still configured per country on the project flow. |
| `defaultLanguage` | string | No | Default enrollee UI / OTP locale when the request omits `language` (for example `en`, `es`). Default: `"en"`. |
