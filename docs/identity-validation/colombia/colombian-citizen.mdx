...
| --- | --- | --- | --- |
| `documentType` | string | Yes | **`CC`** (*Cédula de Ciudadanía*) or **`PPT`** (*Permiso por Protección Temporal*). **`NIT`** and other types are **not** accepted on this endpoint (API validation). |
| `documentNumber` | string | Yes | Document number. **5–10** characters (API validation). Send digits without spaces or periods. |
...
- **Direct premium route:** [National ID premium lookup](/identity-validation/colombia/colombian-cedula-premium) (`/v2/co/cedula/premium`) always uses premium pricing.
...
- Use **`documentType=CC`** for Colombian national ID (*cédula*) and **`PPT`** for *Permiso por Protección Temporal*. Do **not** use **`NIT`** here (that belongs on business/tax endpoints such as [Colombia DIAN Verification](/business-validation/colombia-dian-verification)).
- Some UIs list **`NIT`** next to identity checks by mistake; the public validator for **`/v2/co/cedula`** only allows **`CC`** and **`PPT`**.
