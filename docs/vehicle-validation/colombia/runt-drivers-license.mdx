...
title: RUNT - Driver's License (Premium)
description: 'Verify driver''s licenses in Colombia using the document number with the Verifik API. Omit primerApellido to pay Premium (name lookup included) or send it to pay the Basic rate.'
sidebar_label: RUNT - Driver's License (Premium)
...
export const structuredData = {
	...
	"@type": "TechArticle",
	headline: "Driver's License Validation by Document Number in RUNT | Verifik API Colombia",
	description:
		"Validate driver's licenses in Colombia by document number. Query official RUNT data including license status, categories, and expiration dates.",
	articleSection: "API Documentation",
	keywords: "RUNT Colombia, driver's license validation, license verification, Colombia RUNT API",
};
...
- Omit `primerApellido` — Verifik resolves the last name internally — **Premium** (conductor + name lookup). One history row.
- Send `primerApellido` — name lookup is skipped — **Basic** price (same as [`/v3/co/runt/conductor`](/vehicle-validation/colombia/runt-drivers-license-basic)). One history row.

Use the dedicated [Basic v3](/vehicle-validation/colombia/runt-drivers-license-basic) path if you always send `primerApellido` and want that requirement enforced.
...
-   `PA` is passport, not PPT. Send `primerApellido` when querying a passport; RUNT authenticates drivers with the first last name and masks the full name in the official UI.
