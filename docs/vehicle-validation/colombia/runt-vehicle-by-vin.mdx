...
title: Vehicle Validation by VIN in RUNT | Verifik API Colombia
description: Verify vehicle history and technical specs in Colombia using the VIN (Vehicle Identification Number). Detect fraud and validate vehicle identity with official RUNT data.
...
### 🧠 Vehicle Validation by VIN in RUNT (Colombia)
