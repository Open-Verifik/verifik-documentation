...
description: "Verifica datos de cédula panameña (CCPA): documento, nombre y fecha de nacimiento frente a registros civiles oficiales para KYC y prevención de fraude."
...
export const structuredData = {
	...
	description:
		"Verifica datos de cédula panameña (CCPA): documento, nombre y fecha de nacimiento frente a registros civiles oficiales para KYC y prevención de fraude.",
...
Validación de identidad para personas en **Panamá** con la **CCPA** (*Cédula de Identidad Personal*). Envías **`documentType`** (`CCPA`), **`documentNumber`** normalizado y **`dateOfBirth`** en **`DD/MM/YYYY`**; la API contrasta los datos con fuentes civiles oficiales. Pensada para **KYC**, onboarding y reducción de fraude en flujos totalmente automatizados por HTTPS.
...
Los códigos de provincia oficiales van del **1 al 13**. Códigos inválidos (p. ej. `14`) se rechazan.
