---
id: "es-docs-es-legal-certificate-of-validity-for-legal-professionals-casos-de-uso-3"
title: "Certificado de Vigencia para Profesionales Legales — Casos de Uso"
sourcePath: "docs-es/legal/certificate-of-validity-for-legal-professionals.mdx"
locale: "es"
category: "legal"
tags:
  - "co"
  - "legal"
endpoints:
  - "/v2/co/rama/certificado/vigencia"
  - "/v2/co/rama/certificado/vigencia?documenttype=cc&documentnumber=123456789&quality=abg"
sourceAnchor: "Casos de Uso"
slug: "/legal/certificado-vigencia-profesionales-legales"
url: "https://docs.verifik.co/verifik-es/legal/certificado-vigencia-profesionales-legales"
---

# Certificado de Vigencia para Profesionales Legales
**API path(s):** /v2/co/rama/certificado/vigencia, /v2/co/rama/certificado/vigencia?documenttype=cc&documentnumber=123456789&quality=abg

## Casos de Uso

- **Verificación Profesional**: Verificar credenciales y estado de profesionales legales
- **Debida Diligencia**: Verificar estado profesional durante consultas legales
- **Verificación de Cumplimiento**: Asegurar que los profesionales legales cumplan con requisitos actuales
- **Verificaciones de Antecedentes**: Verificar historial profesional y estado actual
- **Servicios Legales**: Validar profesionales antes de contratar sus servicios

### Notas

- En la API solo son válidos **`CC`** y **`CE`** como **`documentType`** (no NIT).
- Los valores habituales de **`quality`** incluyen **`ABG`**, **`JUEZPAZ`** y **`LT`**.
