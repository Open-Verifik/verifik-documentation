...
description: Consulta em tempo real o Pico y Placa (restrição por final de placa) em Bogotá, Colômbia.
...
Informação **em tempo real** sobre **Pico y Placa** para veículos registrados em **Bogotá**. A restrição usa o **último dígito** da placa. **Exceções** (deficiência ou outras autorizadas) podem aparecer na resposta quando aplicáveis à **`plate`**.
...
| --- | --- | --- | --- |
| `plate` | string | Sim | Placa sem espaços (formato de Bogotá). |
...
- Apenas veículos registrados em **Bogotá**.
- A regra usa o **último dígito** da placa.
- **Autorizações especiais** (deficiência, emergência etc.) podem constar como exceção.
- **`ACTIVO`**: exemplo de exceção vigente.
