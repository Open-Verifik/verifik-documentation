...
- 名前の一部と任意の生年月日が分かっているときは、1:N 検索よりこのヘルパー（または `412 person_already_set` の `data`）を使ってください。どちらも `thumbnails` を含むので、新しい撮影と登録済みの顔を並べて表示できます。
- 名だけでも十分です（`name=Maria`）。同名が多い場合は `date_of_birth` を追加します。
- `POST /v2/face-recognition/persons` の一意性も顔類似度ではなく、コレクション内の **名前 + 生年月日** です。
- `collections[].code` が顔検索の `collection_id` です。
