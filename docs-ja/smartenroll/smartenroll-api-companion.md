...
| ホスト型 SmartEnroll / project flow の許容範囲 | **`0.65`–`0.95`** |
| 直接 face-recognition API（`compare_min_score`） | **`0.67`–`0.95`**（省略時は `0.85`） |

印刷された身分証の肖像は、色褪せ・傷・低解像度であることがよくあります。一部のコロンビア国民ID（cédula）は特に品質が低く、同じ問題は他国でも起こり得ます。ライブのセルフィー対印刷写真は、ライブ同士より**低い**スコアになります。プロジェクトしきい値を **`0.65`** まで下げると、書類品質が悪い正当なユーザーの誤拒否は減りますが、誤受理リスクは上がります。実際のユーザーが失敗しない限り `0.85` を維持してください。
