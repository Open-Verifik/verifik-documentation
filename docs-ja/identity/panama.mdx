...
id: panama
title: パナマ — 国民身分証
description: パナマの CCPA 国民 ID（文書、氏名、生年月日）を公式戸籍ソースに対して検証し、KYC や不正防止に活用できます。
...
export const structuredData = {
	...
	description:
		"パナマの CCPA 国民 ID（文書、氏名、生年月日）を公式戸籍ソースに対して検証し、KYC や不正防止に活用できます。",
	articleSection: "API ドキュメント",
	keywords: "本人確認, KYC API, パナマ CCPA 検証, 公式データソース, 不正防止",
	about: {
		"@type": "Thing",
		name: "本人確認 API",
	},
...
パナマの **CCPA**（*Cédula de Identidad Personal*）データを公式戸籍ソースに対して検証します。**`documentType`**（`CCPA`）、正規化済みの **`documentNumber`**、**`DD/MM/YYYY`** 形式の **`dateOfBirth`** を送信すると、レスポンスは本人確認レコードの整合性を確認し、**KYC**、顧客オンボーディング、不正防止管理に活用できます。
...
| `dateOfBirth`    | string | はい | 生年月日（`DD/MM/YYYY` 形式）。              |
...
フォーマット規則は、オープンソースの [cedula-panama](https://github.com/merlos/cedula-panama) バリデーター（MIT ライセンス）に整合しています。
