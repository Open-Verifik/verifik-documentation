...
パナマの **CCPA**（*Cédula de Identidad Personal*）データを公式戸籍ソースに対して検証します。**`documentType`**（`CCPA`）と正規化済みの **`documentNumber`** を送信すると、レスポンスは本人確認レコードの整合性を確認し、**KYC**、顧客オンボーディング、不正防止管理に活用できます。
